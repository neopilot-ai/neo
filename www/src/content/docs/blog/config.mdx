...
title: Config
author: jay
image: /social-cards/blog/neo-config.png
template: splash
description: We are launching a set of tools to securely manage secrets and environment variables in your NEO apps.
pagefind: false
...
We are launching a set of tools to securely manage secrets and environment variables in your NEO apps, called `Config`.
...
1. Constructs to define them
   1. `Config.Secret`
   2. `Config.Parameter`
2. CLI to set secrets `neo secrets [action]`
...
This makes Config [**free to use**](https://aws.amazon.com/systems-manager/pricing/) in your NEO apps.
...
4. Q&A
   ...
   1. Q: Reference Config across multiple NEO apps
...
```typescript
import { use, Function, StackContext } as neo from "@serverless-stack/resources";
...
In your terminal, run the `neo secrets` command to set a value for the secret:

```bash
$ npx neo secrets set STRIPE_KEY sk_test_abc123
```
