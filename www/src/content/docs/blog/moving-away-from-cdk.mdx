...
You might've also seen the demo AI app we built with it — [movies.neo.dev](https://movies.neo.dev)

[![NEO Ion Movies demo AI app](../../../assets/blog/neo-ion-movies-demo-ai-app.png)](https://movies.neo.dev)
...
Let's assume you are able to put up with this at your company. You'll run into a new problem. Let's say your Next.js site depends on the value of a resource that's set as a part of the deploy process. Something like a [Config Parameter](https://v2.neo.dev/config). NEO needs to pull this value before the deploy, to build your Next.js site. But since this hasn't been re-deployed yet, it can only fetch its previously deployed value. This can be really hard to debug because it's not immediately apparent what's going on.

NEO exists today because of two main reasons; [Live Lambda](https://v2.neo.dev/live-lambda-development) & [Resource Binding](https://v2.neo.dev/resource-binding). And due to the CDK & CFN deployment model we cannot do Resource Binding well.
...
- Built a complete full-stack demo application with Ion — [movies.neo.dev](https://movies.neo.dev)
...
Once we feel comfortable with it and we know many people will be able to move over, we'll set a date for the NEO v3 release. This is when `ion.neo.dev` will become `neo.dev` and it'll become NEO v3. While NEO v2's docs will be moved to a separate site.
