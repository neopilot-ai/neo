---
template: splash
title: Testing in NEO
author: jay
description: You can write tests for your NEO apps using a new CLI that auto-loads the secrets and config.
image: /social-cards/blog/neo-testing.png
...
You can write tests for your NEO apps using the new `neo load-config` CLI. It auto-loads secrets and config for your tests.
...
```bash
$ neo load-config -- vitest run
```

Also, make sure to check out our launch announcement for `Config` in case you missed it.

We updated the `create neo` GraphQL starter to include the updated `npm test` script.

```js
"test": "neo load-config -- vitest run"
```
...
We hosted a [launch livestream on YouTube](https://www.youtube.com/watch?v=YtaxDURRjHA) where we talked about how to write tests for your NEO apps.
...
4. Testing APIs
5. Testing stacks
6. Deep dive into `neo load-config`
7. Q&A
...
To get started, create a new NEO app with our GraphQL starter.

```bash
$ npx create-neo@latest
```
