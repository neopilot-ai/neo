...
template: splash
title: NEO v3
description: Ion is now officially NEO v3.
author: jay
...
Earlier this year we had [announced](/blog/moving-away-from-cdk) that we were working on a new deployment engine for NEO called Ion. After months of testing and a gradual rollout, Ion is now officially being released as NEO v3.

---

## Background

NEO was built on AWS CDK and CloudFormation. But after nearly 3 years of working with CDK and CloudFormation we hit some of its underlying design flaws and were forced to look at alternatives. We talked about this at length in a [**previous blog post**](/blog/moving-away-from-cdk).
...
Fast forward to today, and the number of people actively using Ion is fairly close to the number of people using NEO v2. Ion is also being used in production by a sizeable number of teams.
...
After months of hard work, we are finally here. We are now releasing Ion as NEO v3.
...
Along with the v3 release, we're also making a couple of changes to our websites, how we do support, the NEO Console, and more. In this post we'll go over all of these changes and the migration process.

### New Docs

Starting with the first big change, [ion.neo.khulnasoft.com](https://ion.neo.khulnasoft.com) is now just [neo.khulnasoft.com](https://neo.khulnasoft.com). You might've already noticed the new design.

The docs, [neo.khulnasoft.com/docs](/docs/) are also brand new. The API and reference docs are completely generated from code. This means that they're always up to date and you can view them inline in your code editor.
...
2. **No CDK**
   ...
   There are no CDK constructs. Instead there is an AWS provider from Pulumi that's built on Terraform. These are like your CDK L1 constructs. That said, the Pulumi/Terraform ecosystem is fairly complete. Aside from AWS, NEO now supports [**150+ other providers**](/docs/all-providers#directory).

3. `neo.config.ts`

   The structure of the NEO config has changed a little.
...
4. `neo dev`
   ...
   The `neo dev` CLI now runs a _multiplexer_ that deploys your app and runs your frontends locally. So now you don't have to run `neo dev` and then go start your frontend. You also don't need to wrap your frontend's `dev` script with `neo bind`.
...
These are great for getting started with NEO in general. But we also recommend checking them out even if you've been using v2.

- **Docs**
  
  Start with the [What is NEO](/docs/) and [Workflow](/docs/workflow/) docs. These give you a good overview and lead you to other docs that expand on the various concepts.

- **Guide**

  Our [NEO Guide](https://guide.neo.khulnasoft.com) has been updated to use v3. It teaches you how to build a full-stack app on AWS with NEO.
...
- If you were heavily reliant on L3 CDK Constructs in your NEO apps, it's going to be hard to find direct replacements for those in the Pulumi/Terraform ecosystem.
- There are something that aren't supported yet. But they are on our roadmap.
  ...
  - NEO v3 does not work on Windows yet, you'll need to use WSL.
...
We also streamlined our Discord channels. Instead of having separate channels for the different parts of NEO, you can just post your questions in `#general`.
...
1. **Resources**:
   The Console shows you all the resources in your app, not just the NEO components. This includes the outputs of each of the resources as well.
2. **Updates**:
...
3. **Autodeploy**:
   You can now connect your GitHub repo to the Console and it'll auto-deploy your v3 apps when you _git push_ to it. You can also customize this workflow through your `neo.config.ts`. 

If you are using [Seed](https://seed.run) to deploy your NEO v2 apps, we recommend migrating over to the Console for v3. We are currently not planning to support v3 in Seed.
...
The NEO v2 docs have been moved to [v2.neo.khulnasoft.com](https://v2.neo.khulnasoft.com).
