...
description: NEO now natively supports building containerized applications.
author: jay
...
Historically, NEO has primarily supported deploying serverless applications. But over the last month we've slowly expanded native support for containers on AWS.

<YouTube id="sg4CnvoI7eg" posterQuality="high" />

This includes changes across the entire NEO platform.
...
1. The `dev` prop allows you to run your application locally in a new tab in the `neo dev` multiplexer.

2. The new [`neo tunnel`](/docs/reference/cli#tunnel) command allows your local machine to connect to resources that've been deployed in a VPC. This is helpful because most of the container related components need a VPC. You can [check it out in action here](https://x.com/jayair/status/1844055259729007084).

---

### 3. Console

The [NEO Console](/docs/console) now shows you logs for your containers. And [Autodeploy](/docs/console#autodeploy) will support running in the same VPC as your app. This will allow your deploy process to have access to all the resources in your app.
