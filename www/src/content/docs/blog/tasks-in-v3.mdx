...
You can test your tasks locally in `neo dev` in a similar way to how you test your functions [_Live_](/docs/live/).

<Image src={cliImage} alt="NEO Dev CLI Tasks tab" />
...
<Image src={consoleImage} alt="NEO Console Task logs" />
...
When running in `neo dev`, you are charged for the time it takes to run the task locally as well.
...
If you are coming from NEO v2, there are a couple of differences between `Task` and [`Job`](https://v2.neo.khulnasoft.com/constructs/Job).
