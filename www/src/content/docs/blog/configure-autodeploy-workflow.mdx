...
After you've enabled [Autodeploy](/docs/console#autodeploy), the Console will start a build process, check out your code, detect your package manager, install dependencies, and run `neo deploy`; all without needing a config.
    ...
    autodeploy: {
      async workflow({ $, event }) {
        ...
        event.action === "removed"
          ? await $`npm neo remove`
          : await $`npm neo deploy`;
      }
    }
  }
});
...
Even though the NEO config is in TypeScript, it's still cumbersome to write shell scripts in Node.
...
You can see the new `workflow` prop in action in how the [Console is deployed](https://github.com/neopilot-ai/neo/blob/dev/www/neo.config.ts); since the Console auto-deploys itself!
