...
A universal, standards-based auth provider. It's in beta along with the new [`Auth`](/docs/component/aws/auth) component for NEO v3 that uses this.
...
Auth has been one of the most requested features in NEO. With v1 and v2 of our `Auth` components, we found that most folks wanted a standalone, easy to self-host auth provider.
...
We also think a universal, standards-based auth provider is something that should exist outside of NEO, which is why we built OpenAuth as a separate project.

---

## Using it in NEO

OpenAuth needs some minimal infrastructure to run. The new [`Auth`](/docs/component/aws/auth) component in NEO v3 can do this for you.
...
You can [star the repo](https://github.com/toolbeam/openauth) and [follow us on X](https://x.com/NEO_dev) as we get closer to a 1.0 release.
