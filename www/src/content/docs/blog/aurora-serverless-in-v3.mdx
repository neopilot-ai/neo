...
Aside from scaling to 0, you can also configure the `Aurora` component to not deploy the database in `neo dev`. Instead it can link to your locally running database, if you enable the `dev` prop.

```ts title="neo.config.ts" {3-9}
new neo.aws.Aurora("MyDatabase", {
  engine: "postgres",
  dev: {
    username: "postgres",
