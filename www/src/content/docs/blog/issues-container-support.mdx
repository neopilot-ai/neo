...
import issues from '../../../assets/blog/neo-console-container-issues.png';
...
<Image src={issues} alt="NEO Console Container Issues" />
...
In container applications, your code needs to also import the [NEO JS SDK](/docs/reference/sdk/).
...
Issues works by adding a log subscriber to the CloudWatch Log groups in your NEO apps. This has a filter that matches anything that looks like an error.

In the case of Lambda functions, the Lambda runtime automatically adds a marker to the logs that the filter matches for. For containers, the NEO SDK polyfills the `console` object to add the marker.
