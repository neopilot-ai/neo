...
We also look at how some recent changes have made it possible for NEO to better support them.
...
When we had first started building NEO, deploying frontends was a small part of what we did. But as we grew, we started to get more requests for better frontend support.
...
1. **Break down the problem**
   ...
   So when we wanted to support frontends in NEO, we decided to **separate the two steps**, and open source the adapters.
...
2. **Open source everything**
   ...
   Aside from adapters, even the infrastructure NEO uses to deploy frontends is open source. You can just look at our repo to see how we deploy them.

   This allows people to contribute and makes it so that **NEO supports a large number of frameworks**, covers all their features, and remains up to date with new releases.
...
This made NEO the best available option for people that want to deploy their frontend to their infrastructure.

---

#### Good or good enough?

The infrastructure that NEO creates when you self-host your frontend is **a little different** from what Netlify or Vercel creates.
...
Whereas NEO creates a completely isolated setup for your frontend where nothing is shared between your sites and you are of course isolated from other NEO users.
...
And somewhat related, NEO didn't really support deploying your frontend to multiple regions.
...
1. You can **set up routes** to your frontends.
   ...
   const web = new neo.aws.Nextjs("MyWeb", {
     router: {
       instance: router
     }
   });
...
3. You can share a router across stages. So your **preview environments can deployed almost instantly**.
   ...
   const router = $app.stage === "production"
     ...
     : neo.aws.Router.get("MyRouter", "A2WQRGCYGTFB7Z");
   ```
...
Where NEO shines, is when you start to grow. You start adding multiple frontends. You want serve your API from the same domain. Or you want your preview environments to include more than just your frontend. 

This new setup allows you to do all of that. This is an example of one of NEO's design principles. You can start with the simplest setup, without using a Router. Then grow to adding it. And eventually customize it. All through code.
