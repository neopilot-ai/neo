...
Every NEO app is made up of components. These are logical units that represent features in your app; like your frontends, APIs, databases, or queues.

There are two types of components in NEO:

1. Built-in components — High level components built by the NEO team
...
To fix this, NEO has components that can help you with the most common features in your app.

---

## Built-in

The built-in components in NEO, the ones you see in the sidebar, are designed to make it really easy to create the various parts of your app.
...
:::tip
Aside from the built-in NEO components, all the [Pulumi/Terraform providers](/docs/all-providers#directory) are supported as well.
:::

Currently NEO has built-in components for two cloud providers.
...
:::tip
NEO's built-in components make it easy to build apps with AWS.
:::

These components are namespaced under **`neo.aws.*`** and listed under AWS in the sidebar. Internally they use Pulumi's [AWS](https://www.pulumi.com/registry/packages/aws/) provider.

---

### Cloudflare

These components are namespaced under **`neo.cloudflare.*`** and listed under Cloudflare in the sidebar. Internally they use Pulumi's [Cloudflare](https://www.pulumi.com/registry/packages/cloudflare/) provider.
...
```ts title="neo.config.ts"
const function = new neo.aws.Function("MyFunction", {
...
1. The names of NEO's built-in components and components extended with [`Linkable.wrap`](/docs/component/linkable/#static-wrap) need to be global across your entire app.
...
```ts title="neo.config.ts"
new neo.aws.Function("MyFunctionA", {
  handler: "src/lambdaA.handler"
});

new neo.aws.Function("MyFunctionB", {
...
NEO components evolve over time, sometimes introducing breaking changes. To maintain backwards compatibility, we implement a component versioning scheme.
...
Now if you were using the original `Vpc` component, update NEO, and deploy; you'll get an error during the deploy saying that there's a new version of this component.
...
```diff title="neo.config.ts" lang="ts"
- const vpc = new neo.aws.Vpc("MyVpc");
+ const vpc = new neo.aws.Vpc.v1("MyVpc");
```

Now if you deploy again, NEO knows that you want to stick with the old version and it won't error.
...
```diff title="neo.config.ts" lang="ts"
- const vpc = new neo.aws.Vpc("MyVpc");
+ const vpc = new neo.aws.Vpc("MyNewVpc");
```
...
This is because from NEO's perspective it looks like the `MyVpc` component was removed and a new component called `MyNewVpc` was added.
