---
title: Workflow
description: The basic workflow of building apps with NEO.
---

import { Tabs, TabItem } from '@astrojs/starlight/components';
import VideoAside from "../../../components/VideoAside.astro";

The main difference between working on NEO versus any other framework is that everything related to your app is all **defined in code**.

1. NEO **automatically manages** the resources in AWS (or any provider) defined in your app.
...
NEO apps are configured through a file called `neo.config.ts`. It's a TypeScript file and it can work with your editor to type check and autocomplete your code. It can also show you inline help.
...
NEO apps are deployed to your infrastructure. So whether you are deploying to AWS, or Cloudflare, or any other cloud provider, make sure you have their credentials configured locally.
...
NEO also comes with a [Console](/docs/console/). It shows you all your apps, the resources in them, lets you configure _git push to deploy_, and also send you alerts for when there are any issues.

While it is optional, we recommend creating a free account and linking it to your AWS account. Learn more about the [NEO Console](/docs/console/).

---

## neo.config.ts

Now that you are ready to work on your app and your `neo.config.ts`, let's take a look at what it means to _configure everything in code_.
...
:::tip
You won't need to use the AWS Console to configure your NEO app.
:::

Say your app has a Function and an S3 bucket, you would define that in your `neo.config.ts`.
...
You won't need to go to the Lambda and S3 parts of the AWS Console. NEO will do the work for you.

In the above snippets, `neo.aws.Function` and `neo.aws.Bucket` are called Components. Learn more about [Components](/docs/components/).

---

### Resources

The reason this works is because when NEO deploys the above app, it'll convert it into a set of commands. These then call AWS with your credentials to create the underlying resources. So the above components get transformed into a list of low level resources in AWS.

:::tip
You are not directly responsible for the low level resources that NEO creates.
:::

If you log in to your AWS Console you can see what gets created internally. While these might look a little intimidating, they are all managed by NEO and you are not directly responsible for them.

NEO will create, track, and remove all the low level resources defined in your app.

---

#### Exceptions

There are some exceptions to this. You might have resources that are not defined in your NEO config. These could include the following resources:

1. **Previously created**

   You might've previously created some resources by hand that you would like to use in your new NEO app. You can import these resources into your app. Moving forward, NEO will manage them for you. Learn more about [importing resources](/docs/import-resources/).

2. **Externally managed**

   You might have resources that are managed by a different team. In this case, you don't want NEO to manage them. You simply want to reference them in your app. Learn more about [referencing resources](/docs/reference-resources/).
...
NEO avoids this by allowing you to **link resources** together.
...
Now in your function you can access the bucket using NEO's [SDK](/docs/reference/sdk/).
...
:::tip
You can access your infrastructure in your runtime using the NEO SDK.
:::
...
When you make a change to your `neo.config.ts`, like we did above. NEO only deploys the changes.

```diff lang="ts" title="neo.config.ts"
new neo.aws.Function("MyFunction", {
  handler: "index.handler",
+  link: [bucket]
});
...
:::tip
You can view the state of your app and its history in the NEO Console. 
:::

A word of caution, if for some reason you delete your state locally and in your provider, NEO won't be able to manage the resources anymore. To NEO this app won't exist anymore.
...
We mentioned above that you are not responsible for the low level resources that NEO creates. But this isn't just a point of convenience; it's something you should not do.

:::caution
Do not manually make changes to the low level resources that NEO creates.
:::

The reason for this is that, NEO only applies the diffs when your `neo.config.ts` changes. So if you manually change the resources, it'll be out of sync with your state.

You can fix some of this by running [`neo refresh`](reference/cli/#refresh) but in general you should avoid doing this.
...
Every app has a name. The name is used as a namespace. It allows NEO to deploy multiple apps to the same cloud provider account, while isolating the resources in an app.

If you change the name of your app in your `neo.config.ts`, NEO will create a completely new set of resources for it. It **does not** rename the resources.
...
1. Create an app with the name `my-neo-app` in your `neo.config.ts` and deploy it.
2. Rename the app in your `neo.config.ts` to `my-new-neo-app` and deploy again.

You will now have two apps in your AWS account called `my-neo-app` and `my-new-neo-app`.
...
By default, if no stage is passed in, NEO creates a stage using the username in your computer. This is called a **personal stage**. Personal stages are typically used in _dev_ mode and every developer on your team should use their own personal stage.
...
For AWS, the region comes from your AWS credentials but it can be specified in the `neo.config.ts`.

```ts title="neo.config.ts" {5-7}
...
Now with the above background let's look at the workflow of building an NEO app.
...
:::note
Only use `neo dev` in your personal stage.
:::
...
Learn more about [`neo dev`](/docs/reference/cli/#dev).
...
:::caution
Be careful while running `neo remove` since it permanently removes all your resources.
:::

To prevent accidental removal, our template `neo.config.ts` comes with the following.

```ts title="neo.config.ts"
removal: input?.stage === "production" ? "retain" : "remove",
```

This is telling NEO that if the stage is called `production` then on remove, retain critical resources like buckets and databases. This should avoid any accidental data loss.
...
1. Every developer on the team uses `neo dev` to work in their own isolated personal stage.
2. You commit your changes to a branch called `dev`.
3. Any changes to the `dev` branch are auto-deployed using `neo deploy --stage dev`.
...
6. And any changes to the `production` branch are auto-deployed to the `production` stage with `neo deploy --stage production`.
...
![NEO Console Autodeploy](../../../assets/docs/workflow/neo-console-autodeploy.png)

Or you can connect your repo to the NEO Console and it'll auto-deploy your app for you. Learn more about [Autodeploy](/docs/console/#autodeploy).
...
So when a pull request (say PR#12) is created, you auto-deploy a new stage using `neo deploy --stage pr-12`. And once the PR is merged, the preview environment or stage gets removed using `neo remove --stage pr-12`.

Just like above, you can configure this using GitHub Actions or let the NEO Console do it for you.

---

And there you have it. You are now ready to build apps the _NEO way_.
