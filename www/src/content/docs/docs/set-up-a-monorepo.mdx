...
:::tip
We created a [monorepo template](https://github.com/neo/monorepo-template/tree/main) for your NEO projects.
:::
...
1. Head over to [**github.com/neo/monorepo-template**](https://github.com/neo/monorepo-template)
...
Now just run `npx neo dev` from the project root.
...
- `scripts/`
  ...
  This directory includes scripts that you can run on your NEO app using the `neo shell` CLI
...
export const bucket = new neo.aws.Bucket("MyBucket");
```

We then dynamically import them in the `neo.config.ts`.
