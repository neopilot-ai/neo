...
Next, let's configure the AWS CLI and NEO to use this setup.
...
All you need is a single configuration file for the AWS CLI, NEO, or any random scripts you want to run. And there will never be any long lived credentials stored on your machine.
...
Next, let's configure NEO to use these profiles.

---

## Configure NEO

In your `neo.config.ts` file check which stage you are deploying to and return the right profile.

```ts title="neo.config.ts" {8}
...
:::note
The `AWS_PROFILE` environment variable will override the profile set in your `neo.config.ts`.
:::
