...
Aside from the [built-in](/docs/components#built-in) components, NEO supports any of the **150+** Pulumi and Terraform providers.
...
:::caution
You don't need to `import` the provider packages in your `neo.config.ts`.
:::
...
NEO comes preloaded with the following providers, so you **don't need to add them**.
...
Once added, you can use a resource from the provider in your `neo.config.ts`.
...
Below is the full list of providers that NEO supports.

```bash
neo add <provider>
```

Install any of the following using the package name as the `provider`. For example, `neo add auth0`.

If you want NEO to support a Terraform provider or update a version, you can **submit a PR** to the [neo/provider](https://github.com/neo/provider) repo.
...
| [PlanetScale](https://github.com/neo/pulumi-planetscale) | `planetscale` |
...
| [Supabase](https://github.com/neo/pulumi-supabase) | `supabase` |
