...
description: Migrate your NEO v2 apps to v3.
---
...
This guide will help you migrate your NEO v2 apps to v3. We look at the major differences between v2 and v3 below. But to get a quick intro, we recommend reading the [What is NEO](/docs/) and [Workflow](/docs/workflow/) docs.

:::tip
We recently [migrated our demo notes app](https://github.com/neo/demo-notes-app/pull/8/files) from v2 to v3. You use these changes as reference.
:::
...
NEO v3 has been around for a few months with a pretty sizeable community on <a href={config.discord}>Discord</a>. We've created a channel for folks looking to migrate.
...
|----------|-------|
| `Auth` | [In beta](https://github.com/neopilot-ai/neo/issues/4893) |
| `Script` | [#811](https://github.com/neopilot-ai/neo/issues/4323) |
| `Function` non-Node.js runtimes | [Python](https://github.com/neopilot-ai/neo/issues/4669), [Container](https://github.com/neopilot-ai/neo/issues/4462), [Custom](https://github.com/neopilot-ai/neo/issues/4826) |
...
If you are coming from NEO v2, it's worth starting with the big differences between v2 and v3. It'll help you understand the types of changes you'll need to make as you migrate.

---

#### No CloudFormation

Let's start with the obvious. NEO v3 moves away from CloudFormation and CDK, [we've written in detail about why we decided to do this](https://neo.khulnasoft.com/blog/moving-away-from-cdk.html).

No CloudFormation, means a couple of things:

1. There are no stacks, all the resources are defined through the same function in the `neo.config.ts`.
...
The `neo.config.ts` is similar in v3 but there are some changes. Here's a comparison of the general structure, we look at this in detail in a [section below](#sstconfigts-1).
...
Learn more about the new [`neo.config.ts`](/docs/reference/config/).

---

#### neo dev

The `neo dev` CLI has been completely reworked. It now runs a _multiplexer_ that deploys your app and runs your frontends together. So you don't need to:

- Start your frontend separately
- Need to wrap your frontend `dev` script with `neo bind`
...
There is no `neo build` CLI. Instead you can run `neo diff` to see what changes will be deployed, without doing an actual deploy.

Learn more about [`neo diff`](/docs/reference/cli/#diff).
...
1. Update NEO to v3. Or set the version by hand in your `package.json`. Make sure to this across all the packages in your repo.
...
2. Backup the v2 config with.
   ...
   ```bash frame="none"
   mv neo.config.ts neo.config.ts.bk
   ```
...
4. Set the removal policy to `retain`. Similar to `setDefaultRemovalPolicy` in v2, you can configure the removal policy in `neo.config.ts` in v3.
...
6. Update the dev scripts for your frontend. Remove the `neo bind` from the `dev` script in your `package.json`. For example, for a Next.js app.

   ```diff lang="js" title="package.json"
   - "dev": "neo bind next dev",
   + "dev": "next dev",
   ```
...
Now before we start making changes to our constructs, you might have some stacks code in your `neo.config.ts`.
...
For example, in the [monorepo notes app](https://github.com/neo/demo-notes-app/pull/8) we made these changes.
...
We store our infrastructure files in the `infra/` directory in v3. You can refer to the [demo notes app](https://github.com/neo/demo-notes-app) to see how these are structured.
...
1. **Import the resource**
   ...
   Say the bucket is defined in NEO v2, and the bucket name is `app-prod-MyBucket`.
   ...
   Import is a process of bringing previously created resources into your NEO app and allowing it to manage it moving forward. Learn more about [importing resources](/docs/import-resources/).
...
Finally, when you run `neo remove` for your v2 app, it'll remove the CloudFormation stacks as well.
...
If you are using [**_Seed_**](https://seed.run) to deploy your v2 app; then you'll want to migrate to using [Autodeploy](/docs/console/#autodeploy) on the [NEO Console](/docs/console/). We are currently [not planning to support v3 on Seed](https://seed.run/blog/seed-and-neo-v3).

There are a couple of key reasons to Autodeploy through the Console:

- The builds are run in your AWS account.
- You can configure your workflow through your `neo.config.ts`.
- And you can see which resources were updated as a part of the deploy.

To enable Autodeploy on the Console, you'll need to:

1. Create a new account on the Console — <a href={config.console}>console.neo.khulnasoft.com</a>
...
Listed below are some of the changes to your [`neo.config.ts`](/docs/reference/config/) in general.

---

#### No imports

All the constructs or _components_ are available in the global context. So there's no need to import anything. Your app's `package.json` only needs the `neo` package. There are no extra CDK or infrastructure related packages.

---

#### Globals

There are a couple of global variables, `$app` and `$dev` that replace the `app` argument that's passed into the `stacks()` method of your `neo.config.ts`.
...
5. There is no `app.mode === remove` replacement since your components are not evaluated on `neo remove`.
...
```ts title="neo.config.ts"
$transform(neo.aws.Function, (args, opts) => {
...
Update `neo` to the latest version in your `package.json`. If you have a monorepo, make sure to update `neo` in all your packages.

---

### Bind

In NEO v3, you access all bound or _linked_ resources through the `Resource` module.
...
<Tabs>
  <TabItem label="v3">
  ```ts title="src/lambda.ts" "Resource.MyBucket.name" "neo"
  ...
  <TabItem label="v2">
  ```ts title="src/lambda.ts" "Bucket.MyBucket.bucketName" "neo/node/bucket"
  import { Bucket } from "neo/node/bucket";
...
<Tabs>
  <TabItem label="v3">
  ```ts title="src/lambda.ts" "Resource.MySecret.value" "neo"
  import { Resource } from "neo";
  ...
  <TabItem label="v2">
  ```ts title="src/lambda.ts" "Config.MySecret" "neo/node/config"
  import { Config } from "neo/node/config";
...
In v2, some modules in the Node client had [handlers and hooks](https://v2.neo.khulnasoft.com/clients#handlers).

```ts title="v2"
import { ApiHandler } from "neo/node/api";
...
This means that you have both v2 and v3 installed in your project. Since, they both have an `neo` binary, you want to make sure v3 takes precedence. So v3 should be listed **after** v2.
...
```ts title="v3"
import { ApiHandler } from "neov2/node/api";
...
2. Since `Task` is natively based on Fargate, you can use the AWS SDK to interact with it, even in runtimes the NEO SDK doesn't support.
...
<Tabs>
  <TabItem label="v3">
  ```bash "secret"
  npx neo secret set MySecret sk_test_abc123
  ```
  </TabItem>
  <TabItem label="v2">
  ```bash "secrets"
  npx neo secrets set MySecret sk_test_abc123
  ```
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="v3">
  ```ts title="neo.config.ts"
  const secret = new neo.Linkable("MyParameter", {
...
<Tabs>
  <TabItem label="v3">
  ```ts title="neo.config.ts"
  const cluster = new neo.aws.Cluster("MyCluster", {
    vpc: {
