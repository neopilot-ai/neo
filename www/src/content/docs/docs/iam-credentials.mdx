...
NEO deploys your AWS resources using your AWS credentials. In this guide we'll look at how to set these credentials, the basic set of permissions it needs, and how to customize it.
...
1. Follow this to [create an IAM user](https://neo.khulnasoft.com/chapters/create-an-iam-user.html)
2. And then use this to [configure the credentials](https://neo.khulnasoft.com/chapters/configure-the-aws-cli.html)
...
By default, NEO uses the credentials for the `default` profile. To use one of the other profiles, set the `profile` in your `neo.config.ts`.
...
If you've configured AWS credentials previously through the `AWS_PROFILE` environment variable or through a `.env` file, it will override the profile set in your `neo.config.ts`. So make sure to remove any references to `AWS_PROFILE`.

---

#### From environment variables

NEO can also detect AWS credentials in your environment and use them to deploy.
...
If you have AWS credentials set in multiple places, NEO will first look at:
...
2. NEO config
   ...
   Then it'll check for the credentials or `profile` in your `neo.config.ts`.
...
However, if you are using NEO at your company, you want to secure these permissions. Here we'll look at exactly what NEO needs and how you can go about customizing it.
...
```json title="iam-policy.json"
{
    ...
    "Statement": [
      ...
      },
      {
          ...
          "Resource": [
              "arn:aws:ecr:REGION:ACCOUNT:repository/neo-asset"
          ]
      },
      {
          ...
          "Resource": [
              "arn:aws:ssm:REGION:ACCOUNT:parameter/neo/passphrase/*",
              "arn:aws:ssm:REGION:ACCOUNT:parameter/neo/bootstrap"
          ]
      },
      {
          "Sid": "Deployments",
      ...
      },
      {
          ...
          "Resource": [
              "arn:aws:ssm:REGION:ACCOUNT:parameter/neo/*"
          ]
      },
      {
...
1. Permissions needed to bootstrap NEO in your AWS account
...
NEO needs to [bootstrap](/docs/state/#bootstrap) each AWS account, in each region, once. This happens automatically when you run `neo deploy` or `neo dev`.
...
The permissions that NEO needs to deploy the resources in your app, depends on what you have in your app.
...
The NEO CLI also makes some AWS SDK calls to your account. Here are the IAM permissions it needs.
...
- Permissions to manage your [secrets](/docs/component/secret).
  ...
  ```json
  {
    ...
    "Resource": [
      "arn:aws:ssm:us-east-1:112233445566:parameter/neo/*"
    ]
  }
  ```

- And permissions to connect to the IoT endpoint in `neo dev` to run your functions [_Live_](/docs/live).
