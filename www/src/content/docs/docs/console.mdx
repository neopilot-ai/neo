---
title: Console
description: Manage and monitor your apps with the NEO Console.
---

import { Image } from "astro:assets"
import { Tabs, TabItem } from '@astrojs/starlight/components';

import consoleHomeLight from '../../../assets/docs/console/neo-console-home-light.png';
import consoleHomeDark from '../../../assets/docs/console/neo-console-home-dark.png';
import consoleLogsLight from '../../../assets/docs/console/neo-console-logs-light.png';
import consoleLogsDark from '../../../assets/docs/console/neo-console-logs-dark.png';
import consoleIssuesLight from '../../../assets/docs/console/neo-console-issues-light.png';
import consoleIssuesDark from '../../../assets/docs/console/neo-console-issues-dark.png';
import consoleLocalLight from '../../../assets/docs/console/neo-console-local-light.png';
import consoleLocalDark from '../../../assets/docs/console/neo-console-local-dark.png';
import consoleResourcesLight from '../../../assets/docs/console/neo-console-resources-light.png';
import consoleResourcesDark from '../../../assets/docs/console/neo-console-resources-dark.png';
import consoleUpdatesLight from '../../../assets/docs/console/neo-console-updates-light.png';
import consoleUpdatesDark from '../../../assets/docs/console/neo-console-updates-dark.png';
import consoleAutodeployLight from '../../../assets/docs/console/neo-console-autodeploy-light.png';
import consoleAutodeployDark from '../../../assets/docs/console/neo-console-autodeploy-dark.png';

The Console is a web based dashboard to manage your NEO apps — [**console.neo.khulnasoft.com**](https://console.neo.khulnasoft.com)
...
<a href="https://console.neo.khulnasoft.com" target="_blank" rel="noopener noreferrer">
  <picture>
    ...
    <Image src={consoleHomeLight} alt="NEO Console" />
  </picture>
</a>
...
1. **Create an account with your email**
   ...
   It's better to use your work email so that you can invite your team to your workspace later — [**console.neo.khulnasoft.com**](https://console.neo.khulnasoft.com)
...
2. **Connect your AWS account**
   ...
   This stack will scan all the regions in your account for NEO apps and subscribe to them. Once created, you'll see all your apps, stages, and the functions in the apps.
...
- You can view all your apps and stages
  ...
  Once you've connected your AWS accounts, it'll deploy a separate CloudFormation stack and connect to any NEO apps in it. And all your apps and stages will show up automatically.

- It's open-source and built with NEO

  The Console is an NEO app. You can view the [source on GitHub](https://github.com/neo/console). It's also auto-deployed using itself.
...
- **Read Permissions**: The Console needs specific permissions to display information about resources within your NEO apps.
...
2. Edit the template file with necessary changes.
   ...
   <summary>_View the template changes_</summary>
     ...
     +                 ],
     +                 "Resource": {
     +                   "Fn::Sub": "arn:aws:events:*:${AWS::AccountId}:rule/NEOConsole*"
     +                 }
     +               },
     +               {
     +                 "Effect": "Allow",
     ...
     +                 ],
     +                 "Resource": {
     +                   "Fn::Sub": "arn:aws:iam::${AWS::AccountId}:role/NEOConsolePublisher*"
     +                 }
     +               },
     +               {
     +                 "Effect": "Allow",
...
[Starting Feb 1, 2025](/blog/console-pricing-update), the Console will be priced based on the number of active resources in your NEO apps.
...
- A resource is what NEO creates in your cloud provider. [Learn more below](#faq).
...
Previously, the Console pricing was based on the number of times the Lambda functions in your NEO apps are invoked per month and it used the following tiers.
...
3. [**Local logs**](#local-logs): View logs from your local `neo dev` session
...
<picture>
  ...
  <Image src={consoleLogsLight} alt="NEO Console Logs" />
</picture>
...
<picture>
  ...
  <Image src={consoleIssuesLight} alt="NEO Console Issues" />
</picture>
...
- **Containers**
  ...
  In a container applications, your code needs to also import the [NEO JS SDK](/docs/reference/sdk/).
...
1. When an app is deployed or when an account is first synced, we add a log subscriber to the CloudWatch Log groups in your NEO apps.
...
2. If the subscriber filter matches anything that looks like an error it invokes the Lambda function.
   ...
   - For containers, the NEO SDK polyfills the `console` object to add the marker.
...
4. It then hits an endpoint in the NEO Console and passes in that error.
...
These are added to **every region** in your AWS account that has a CloudWatch log group from your NEO apps. It's deployed using a CloudFormation stack.
...
Each update in your app also gets a unique URL, a **_permalink_**. This is printed out by the NEO CLI.

```bash title="neo deploy"
↗  Permalink  https://neo.khulnasoft.com/u/318d3879 
```
...
<picture>
  ...
  <Image src={consoleUpdatesLight} alt="NEO Console Updates" />
</picture>
...
<picture>
  ...
  <Image src={consoleResourcesLight} alt="NEO Console Resources" />
</picture>
...
<picture>
  ...
  <Image src={consoleAutodeployLight} alt="NEO Console Autodeploy" />
</picture>

We designed Autodeploy to be a better fit for NEO apps when compared to alternatives like GitHub Actions or CircleCI.
...
2. **Configurable**
   - You can configure how Autodeploy works directly through your `neo.config.ts`.
...
:::tip
You can configure how Autodeploy works through your `neo.config.ts`.
:::

While Autodeploy supports the standard branch and PR workflow out of the box, it can be configured in depth through your `neo.config.ts`.

---

#### Configure

The above can be configured through the [`console.autodeploy`](/docs/reference/config/#console-autodeploy) option in the `neo.config.ts`.

```ts title="neo.config.ts" {7-15}
...
1. **Stage**
   ...
   As mentioned, above you can customize this through your `neo.config.ts`.
...
3. **Environment Variables**
   ...
   Any environment variables you need for the build process. These are made available under `process.env.*` in your `neo.config.ts`.
...
When the Console starts up, it checks if you are running `neo dev` locally. If so, then it'll show you real-time logs from your local terminal. This works by connecting to a local server that's run as a part of the NEO CLI.
...
<picture>
  ...
  <Image src={consoleLocalLight} alt="NEO Console Local logs" />
</picture>

:::info
The local server only allows access from `localhost` and `console.neo.khulnasoft.com`.
:::
...
Certain browsers like Safari and Brave require the local connection between the browser and the `neo dev` CLI to be running on HTTPS.

NEO can automatically generate a locally-trusted certificate using the [`neo cert`](/docs/reference/cli#cert) command.
...
If you are using [Gitpod](https://www.gitpod.io/), you can use the Gitpod Local Companion app to connect to the `neo dev`  process running inside your Gitpod workspace.
...
- Do I need to use the Console to use NEO?

  You **don't need the Console** to use NEO. It compliments the CLI and has some features that help with managing your apps in production.
...
- What happens if I go over the free tier?
  ...
  Note that, you can continue to **access your personal stages**. Just make sure you have `neo dev` running locally. Otherwise the Console won't be able to detect that it's a personal stage.

- What counts as a resource?

  Resources are what NEO creates in your cloud provider. This includes the resources created by both NEO's built-in components, like `Function`, `Nextjs`, `Bucket`, and the ones created by any other Terraform/Pulumi provider.
  ...
  For some context, the Console is itself a pretty large [NEO app](https://github.com/neo/console) and it has around 320 resources.
...
- Which Lambda functions are included in the number of invocations?
  ...
  The number of invocations are only counted for the **Lambda functions in your NEO apps**. Other Lambda functions in your AWS accounts are not included.
...
[contact-us]: mailto:hello@neo.khulnasoft.com
