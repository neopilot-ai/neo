---
title: Send emails in AWS with NEO
description: Send emails from your API in AWS with NEO.
---

We are going to build a simple NEO app in AWS with a serverless API, and send emails from it.

:::tip[View source]
You can [view the source](https://github.com/neopilot-ai/neo/tree/dev/examples/aws-email) of this example in our repo.
:::
...
Let's add Email to our app, it uses [Amazon SES](https://aws.amazon.com/ses/) behind the scenes. Update your `neo.config.ts`.

```js title="neo.config.ts" {3}
async run() {
...
Next let's create a simple API that'll send out an email when invoked. Add this to your `neo.config.ts`.
...
![Verify your email with NEO](../../../../../assets/docs/start/verify-your-email-with-neo.png)
...
![Email sent from NEO](../../../../../assets/docs/start/email-sent-from-neo.png)
