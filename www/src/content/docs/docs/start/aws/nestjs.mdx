---
title: NestJS on AWS with NEO
description: Create and deploy an NestJS app to AWS with NEO.
---

We are going to build an app with NestJS, add an S3 Bucket for file uploads, and deploy it to AWS in a container with NEO.

:::tip[View source]
You can [view the source](https://github.com/neopilot-ai/neo/tree/dev/examples/aws-nestjs-container) of this example in our repo.
:::
...
Also make sure you have Node 22.12. Or set the `--experimental-require-module` flag. This'll allow NestJS to import the NEO SDK.
...
This'll create a `neo.config.ts` file in your project root.

To make sure the types in the `neo.config.ts` are picked up, add the following to the `tsconfig.json`.

```diff lang="json" title="tsconfig.json"
{
+ "include": ["src/**/*", "test/**/*", "neo-env.d.ts"]
}
```
...
To deploy our Nest app, let's add an [AWS Fargate](https://aws.amazon.com/fargate/) container with [Amazon ECS](https://aws.amazon.com/ecs/). Update your `neo.config.ts`.
...
The `dev.command` tells NEO to instead run our Nest app locally in dev mode.

---

#### Start dev mode

Run the following to start dev mode. This'll start NEO and your Nest app.
