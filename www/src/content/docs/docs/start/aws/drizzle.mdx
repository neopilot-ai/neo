---
title: Drizzle with Amazon RDS and NEO
description: Use Drizzle and NEO to manage and deploy your Amazon Postgres RDS database.
---

You can use NEO to deploy an Amazon Postgres RDS database and set up [Drizzle ORM](https://orm.drizzle.team) and [Drizzle Kit](https://orm.drizzle.team/docs/kit-overview) to manage it.


:::tip[View source]
You can [view the source](https://github.com/neopilot-ai/neo/tree/dev/examples/aws-drizzle) of this example in our repo.
:::
...
```json title="package.json" "neo shell"
"scripts": {
  "db": "neo shell drizzle-kit"
},
```

The `neo shell` CLI will pass the credentials to Drizzle Kit and allow it to connect to your database.
...
Let's add a Postgres database using [Amazon RDS](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Welcome.html). This needs a VPC. Update your `neo.config.ts`.
...
When you run NEO in dev it can start other dev processes for you. In this case we want to start Drizzle Studio. Add this below the `Postgres` component.
...
We'll use a Lambda function as an API to query our database. Add the following to your `neo.config.ts` below the database config.
...
:::tip
NEO allows us to automatically access our database with `Resource.MyPostgres.*`.
:::
...
This in turn runs `neo shell drizzle-kit generate` and creates a new migration in the `migrations/` directory.
...
To see our schema in action we can open the Drizzle Studio. Head over to the **Studio** tab in your `neo dev` session and go to the link.

Or head over to `https://local.drizzle.studio` in your browser.

![Initial Drizzle Studio with NEO](../../../../../assets/docs/start/initial-drizzle-studio-with-neo.png)
...
![Todo created with Drizzle in NEO](../../../../../assets/docs/start/todo-created-with-drizzle-in-neo.png)
