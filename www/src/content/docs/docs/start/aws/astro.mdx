---
title: Astro on AWS with NEO
description: Create and deploy an Astro site to AWS with NEO.
---

There are two ways to deploy an Astro site to AWS with NEO.
...
:::tip[View source]
You can [view the source](https://github.com/neopilot-ai/neo/tree/dev/examples/aws-astro) of this example in our repo.
:::
...
Run the following to start dev mode. This'll start NEO and your Astro site.
...
![NEO Astro app local](../../../../../assets/docs/start/start-astro-local.png)
...
:::tip[View source]
You can [view the source](https://github.com/neopilot-ai/neo/tree/dev/examples/aws-astro-container) of this example in our repo.
:::
...
To deploy our Astro site in a container, we'll use [AWS Fargate](https://aws.amazon.com/fargate/) with [Amazon ECS](https://aws.amazon.com/ecs/). Replace the `run` function in your `neo.config.ts`.

```ts title="neo.config.ts" {10-12}
async run() {
...
The `dev.command` tells NEO to instead run our Astro site locally in dev mode.
...
![NEO Astro app local](../../../../../assets/docs/start/start-astro-local-container.png)
