---
title: Hono on AWS with NEO
description: Create and deploy a Hono API in AWS with NEO.
---

There are two ways to deploy a [Hono](https://hono.dev) app to AWS with NEO.
...
:::tip[View source]
You can [view the source](https://github.com/neopilot-ai/neo/tree/dev/examples/aws-hono) of this example in our repo.
:::
...
Let's add a Hono API using an AWS Lambda. Update your `neo.config.ts`.
...
Let's add an S3 Bucket for file uploads. Update your `neo.config.ts`.
...
:::tip[View source]
You can [view the source](https://github.com/neopilot-ai/neo/tree/dev/examples/aws-hono-container) of this example in our repo.
:::
...
The `dev.command` tells NEO to instead run our Hono app locally in dev mode.

---

#### Start dev mode

Run the following to start dev mode. This'll start NEO and your Hono app.
