---
title: Realtime apps in AWS with NEO
description: Use NEO to build and deploy a realtime chat app to AWS.
---

We are going to use NEO to build and deploy a realtime chat app on AWS.

:::tip[View source]
You can [view the source](https://github.com/neopilot-ai/neo/tree/dev/examples/aws-realtime-nextjs) of this example in our repo.
:::
...
Let's add the `Realtime` component and link it to the Next.js app. Update your `neo.config.ts`.

```js title="neo.config.ts" {7}
async run() {
...
Here we are going to publish and subscribe to a _topic_ called `neo-chat`, namespaced under the name of the app and the stage our app is deployed to.
...
![NEO Realtime Next.js app](../../../../../assets/docs/start/neo-realtime-nextjs-app.png)
