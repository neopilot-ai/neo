---
title: tRPC on Cloudflare with NEO
description: Create and deploy a tRPC API in Cloudflare with NEO.
---

We are going to build a [tRPC](https://trpc.io) API, a simple client, and deploy it to Cloudflare using NEO.

:::tip[View source]
You can [view the source](https://github.com/neopilot-ai/neo/tree/dev/examples/cloudflare-trpc) of this example in our repo.
:::
...
Let's add two Workers; one for our tRPC server and one that'll be our client. Update your `neo.config.ts`.
...
```bash frame="none"
+  Complete
   api: https://my-trpc-app-jayair-trpcscript.neo-15d.workers.dev
   client: https://my-trpc-app-jayair-clientscript.neo-15d.workers.dev
```
...
```bash
curl https://my-trpc-app-jayair-clientscript.neo-15d.workers.dev
```
