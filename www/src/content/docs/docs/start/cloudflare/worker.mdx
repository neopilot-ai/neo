---
title: Cloudflare Workers with NEO
description: Create and deploy a Cloudflare Worker as an API with NEO.
---

We are going to build an API with a Cloudflare Worker, add an R2 bucket for file uploads, and deploy it using NEO.

:::tip[View source]
You can [view the source](https://github.com/neopilot-ai/neo/tree/dev/examples/cloudflare-worker) of this example in our repo.
:::
...
Select the defaults and pick **Cloudflare**. This'll create a `neo.config.ts` file in your project root.
...
Let's add a Worker. Update your `neo.config.ts`.
...
Let's add an R2 bucket for file uploads. Update your `neo.config.ts`.
...
```bash frame="none"
+  Complete
   api: https://start-cloudflare-jayair-myworkerscript.neo-15d.workers.dev
```
...
```bash
curl --upload-file package.json https://start-cloudflare-jayair-myworkerscript.neo-15d.workers.dev
```

Now head over to `https://start-cloudflare-jayair-myworkerscript.neo-15d.workers.dev` in your browser and it'll load the file you just uploaded.
