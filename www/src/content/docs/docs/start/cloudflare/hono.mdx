---
title: Hono on Cloudflare with NEO
description: Create and deploy a Hono API on Cloudflare with NEO.
---

We are going to build an API with Hono, add an R2 bucket for file uploads, and deploy it using Cloudflare with NEO.

:::tip[View source]
You can [view the source](https://github.com/neopilot-ai/neo/tree/dev/examples/cloudflare-hono) of this example in our repo.
:::
...
```bash frame="none"
✓  Complete
   Hono: https://my-hono-api-jayair-honoscript.neo-15d.workers.dev
```
...
```bash
curl -X PUT --upload-file package.json https://my-hono-api-jayair-honoscript.neo-15d.workers.dev
```

Now head over to `https://my-hono-api-jayair-honoscript.neo-15d.workers.dev` in your browser and it'll load the file you just uploaded.
