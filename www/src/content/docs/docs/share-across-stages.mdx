...
You define all the resources in your app in your `neo.config.ts`. These resources then get created for each stage that you deploy to.
...
To help with this some NEO components come with a `static get` method. These components are typically ones that people want to be able to share. Here are some components that have this:
...
And it'll print it out on `neo deploy`.
