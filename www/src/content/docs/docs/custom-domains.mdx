...
You can configure custom domains and subdomains for your frontends, APIs, services, or routers in NEO.

:::note
NEO currently supports configuring custom domains for AWS components.
:::
...
NEO supports a couple of DNS providers automatically. These include AWS Route 53, Cloudflare, and Vercel. Other providers will need to be manually configured.
...
NEO can perform these steps automatically for the supported providers through a concept of _adapters_. These create the above DNS records on a given provider.

---

## Adapters

You can use a custom domain hosted on any provider. NEO supports domains on AWS, Cloudflare, and Vercel automatically.

---

### AWS

By default, if you set a custom domain, NEO assumes the domain is configured in AWS Route 53 in the same AWS account.
...
This is the same as using the [`neo.aws.dns`](/docs/component/aws/dns/) adapter.
...
3. Use the [`neo.vercel.dns`](/docs/component/vercel/dns/) adapter.
...
2. Set the **`CLOUDFLARE_API_TOKEN`** in your environment.
   ...
   The Cloudflare providers need these credentials to deploy your app in the first place, which means they can't be set using the `neo secret` CLI.

   If you are auto-deploying your app through the [NEO Console](console.mdx#autodeploy) or through your CI, you'll need to set these as environment variables.


3. Use the [`neo.cloudflare.dns`](/docs/component/cloudflare/dns/) adapter.
