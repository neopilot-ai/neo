...
Live is a feature of NEO that lets you test changes made to your AWS Lambda functions in milliseconds. Your changes work without having to redeploy. And they can be invoked remotely.

:::tip
By default, `neo dev` will run all the functions in your app _"live"_.
:::
...
When you run `neo dev`, it [bootstraps](/docs/state#bootstrap) a new AppSync Events API for the region you are using.

This is roughly what the flow looks like:

1. When you run `neo dev`, it deploys your app and replaces the Lambda functions with a _stub_ version.
...
2. **Live mode persists**
   ...
   If you kill the `neo dev` CLI, your functions are not run locally anymore but the stub function in AWS are still there. This means that it'll attempt to proxy requests to your machine and timeout.

   :::tip
   Only use `neo dev` in your personal stage.
   :::

   You can fix this by running `neo deploy` and it'll deploy the real version of your app. But the next time you run `neo dev` it'll need to deploy the stub back. This'll take a couple of minutes. So we recommend only using your personal stages for `neo dev`. And avoid flipping back and forth between `dev` and `deploy`.

---

### Live mode

When a function is running live it sets the `NEO_DEV` environment variable to `true`. So in your Node.js functions you can access it using `process.env.NEO_DEV`.

```js title="src/lambda.js" "process.env.NEO_DEV"
export async function main(event) {
  const body = process.env.NEO_DEV ? "Hello, Live!" : "Hello, World!";
...
For example, when running locally you might want to connect to a local database. You can do that with the `NEO_DEV` environment variable.

```js
const dbHost = process.env.NEO_DEV
  ? "localhost"
...
1. Enable the `bastion` host in your VPC.
   ...
   ```ts title="neo.config.ts" "{ bastion: true }"
   new neo.aws.Vpc("MyVpc", { bastion: true });
   ```
...
2. Install the tunnel.
   ...
   This needs _sudo_ to create the network interface on your machine. You only need to do this once.
    ...
    :::note
    For NixOS users you will also need to declare the following sudo configuration to complete the setup:
      ...
      security.sudo.extraRules = [
        {
          ...
          commands = [
            {
              command = "/opt/neo/tunnel tunnel start *";
...
Now open a new terminal in VS Code, run `neo dev`, set a breakpoint in a function, and invoke the function.

---

## Changelog

Live is a feature that was created by NEO when it first launched back in 2021. It's gone through a few different iterations since then.
