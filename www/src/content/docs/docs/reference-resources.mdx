...
Referencing is the process of _using_ some externally created resources in your NEO app; without having NEO manage them.

This is for when you have some resources that are either managed by a different team or a different IaC tool. Typically these are low-level resources and not NEO's built-in components.

There are a few different ways this shows up in NEO.

---

## Lookup a resource

Let's say you have an existing resource that you want to use in your NEO app.
...
Here we are assuming the bucket wasn't created through an NEO app. This is why we are using the low-level `aws.s3.BucketV2`. If this was created in an NEO app or in another stage in the same app, there's a similar `static neo.aws.Bucket.get` method. Learn more about [sharing across stages](/docs/share-across-stages).

---

#### How it works

When you create a resource in your NEO app, two things happen. First, the resource is created by making a bunch of calls to your cloud provider. Second, NEO makes a call to _get_ the resource from the cloud provider. The data that it gets back is stored in your [state](/docs/state/).
...
You can use the [`neo.Linkable`](/docs/component/linkable/) component, to be able to link any property of this resource.
...
Aside from looking up resources, you can also pass existing resources in to the built-in NEO components. This is typically when you are trying to create a new resource and it takes another resource as a part of it.
...
NEO is simply asking for the props the underlying resource needs. In this case, it only needs the function ARN.
...
```ts title="neo.config.ts" {3}
neo.aws.Queue.subscribe("arn:aws:sqs:us-east-1:123456789012:MyQueue", "src/subscriber.handler");
```
...
There are a few other built-in NEO components that have `static` methods like this.
...
With this you can continue to manage the root resource outside of NEO, while still being able to attach to them.
