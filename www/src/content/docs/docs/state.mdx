...
When you deploy your app, NEO creates a state file locally to keep track of the state of the infrastructure in your app.

So when you make a change, it'll allow NEO to do a diff with the state and only deploy what's changed.
...
Aside from these, NEO also creates some other resources when your app is first deployed. We'll look at this below. 

---

The state is generated locally but is backed up to your provider using:

1. A **bucket** to store the state, typically named `neo-state-<hash>`. This is created in the region of your `home`. More on this below.
2. An **SSM parameter** to store the passphrase used to encrypt your secrets, under `/neo/passphrase/<app>/<stage>`. Also created in the region of your `home`.

:::danger
Do not delete the SSM parameter that stores the passphrase for your app.
:::

The passphrase is used to encrypt any secrets and sensitive information. Without it NEO won't be able to read the state file and deploy your app.

---

## Home

Your `neo.config.ts` specifies which provider to use for storing your state. We call this the `home` of your app.
...
When you specify your home provider, NEO assumes you'd like to use that provider in your app as well and adds it to your providers internally. So the above is equivalent to doing this.
...
As NEO starts deploying the resources in your app, it creates some additional _bootstrap_ resources. If your app has a Lambda function or a Docker container, then NEO will create the following in the same region as the given resource:

1. An assets bucket to store the function packages, typically named `neo-asset-<hash>`.
2. An ECR repository to store container images, called `neo-asset`.
3. An SSM parameter to store the assets bucket name and the ECR repository, stored under `/neo/bootstrap`.
...
When you remove an NEO app, it does not remove the _state_ or _bootstrap_ resources. This is because it does not know if there are other apps that might be using this. So if you want to completely remove any NEO created resources, you'll need to manually remove these in the regions you've deployed to.

---

### Reset

If you accidentally remove the bootstrap resources the NEO CLI will not be able to start up.
...
Now when you run the NEO CLI, it'll bootstrap your account again.
...
1. The components in the `neo.config.ts` get converted into low level resource definitions. These get compared to the the state file.
...
3. The state file is updated to reflect the new state of these resources. Now the `neo.config.ts`, the state file, and the resources in the cloud provider are all in sync.
...
- NEO will make a call to AWS to set the timeout of the resource to 30 seconds.
...
- NEO will make a call to update the timeout of the resource to 60 seconds.
...
- So NEO will make a call to AWS to create a new function with the given config.

In general we do not recommend manually changing resources in a cloud provider since it puts your state out of sync. But if you find yourself in a situation where this happens, you can use the `neo refresh` command to put them back in sync.
