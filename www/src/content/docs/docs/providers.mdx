...
A provider is what allows NEO to interact with the APIs of various cloud services. These are packages that can be installed through your `neo.config.ts`.

<VideoAside title="Watch a video about providers in NEO" href="https://youtu.be/rlR2f5N9mW4" />

NEO is built on Pulumi/Terraform and **supports 150+ providers**. This includes the major clouds like AWS, Azure, and GCP; but also services like Cloudflare, Stripe, Vercel, Auth0, etc.
...
NEO manages these packages internally and you do not need to import the package in your `neo.config.ts`.

:::tip
Your app can have multiple providers.
:::

The name of a provider comes from the **name of the package** in the [Directory](/docs/all-providers#directory). For example, `neo add planetscale`, will add the following to your `neo.config.ts`.
...
Read more about the [`neo add`](/docs/reference/cli/#add) command.

---

## Configure

You can configure a provider in your `neo.config.ts`. For example, to change the region for AWS.
...
By default, NEO installs the latest version. If you want to use a specific version, you can change it in your config.
...
If you make any changes to the `providers` in your config, you'll need to run `neo install`.

:::tip
You'll need to run `neo install` if you update the `providers` in your config.
:::

The version of the provider is always pinned to what's in the `neo.config.ts` and does not auto-update. This is the case, even if there is no version set. This is to make sure that the providers don't update in the middle of your dev workflow.

:::note
Providers don't auto-update. They stick to the version that was installed initially.
:::

So if you want to update it, you'll need to change it manually and run `neo install`.
...
For example, running `neo add aws` will allow you to use all the components under the `aws` namespace.
...
Aside from components in the providers, NEO also has a list of built-in components. These are typically higher level components that make it easy to add features to your app.
...
You can create multiple instances of a provider that's in your config. By default NEO creates one instance of each provider in your `neo.config.ts` with the defaults. By you can create multiple instances in your app.  
...
```ts {1} title="neo.config.ts" "{ provider: useast1 }"
const useast1 = new aws.Provider("useast1", { region: "us-east-1" });

new neo.aws.Function("MyFunction, "src/lambda.handler");
