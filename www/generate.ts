...
function generateCliDoc() {
  ...
  fs.writeFileSync(
    outputFilePath,
    [
      renderHeader("CLI", "Reference doc for the NEO CLI."),
      renderSourceMessage("cmd/neo/main.go"),
      ...
      cmd.children
        ...
        .flatMap((subcmd) => {
          ...
          lines.push(
            `<Section type="signature">`,
            '```sh frame="none"',
            `neo ${cmd.name} ${renderCliCommandUsage(subcmd)}`,
            "```",
            `</Section>`
          );
...
function generateCommonErrorsDoc() {
  ...
  function renderCommonErrorsAbout() {
    return [
      "Below is a collection of common errors you might encounter when using NEO.",
      "",
      ":::tip",
...
async function generateExamplesDocs() {
  ...
  function renderIntro() {
    return [
      `Below is a collection of example NEO apps. These are available in the [\`examples/\`](${config.github}/tree/dev/examples) directory of the repo.`,
      "",
      ":::tip",
      "This doc is best viewed through the site search or through the _AI_.",
      ":::",
      "",
      "The descriptions for these examples are generated using the comments in the `neo.config.ts` of the app.",
      "",
      "#### Contributing",
      `To contribute an example or to edit one, submit a PR to the [repo](${config.github}).`,
      "Make sure to document the `neo.config.ts` in your example.",
      "",
    ];
  }
...
async function generateConfigDoc(module: TypeDoc.DeclarationReflection) {
  ...
  fs.writeFileSync(
    outputFilePath,
    [
      renderHeader("Config", "Reference doc for the `neo.config.ts`."),
...
async function generateDnsDoc(module: TypeDoc.DeclarationReflection) {
  ...
  fs.writeFileSync(
    ...
    [
      renderHeader(
        `${title} DNS Adapter`,
        `Reference doc for the \`neo.${dnsProvider}.dns\` adapter.`
      ),
      renderSourceMessage(sourceFile),
      renderImports(outputFilePath),
      renderBodyBegin(),
...
async function generateLinkableDoc(module: TypeDoc.DeclarationReflection) {
  ...
  const copy = {
    "components/aws/permission": {
      title: "AWS",
      namespace: "neo.aws.permission",
    },
    "components/cloudflare/binding": {
      title: "Cloudflare",
      namespace: "neo.cloudflare.binding",
    },
  }[module.name]!;
...
function renderType(
  ...
  // ie. NEO SDK uses @see [@aws-sdk/client-ecs.DescribeTasksResponse](https://docs.aws.amazon.com/AWSJavaScriptSDK/v3/latest/Package/-aws-sdk-client-ecs/Interface/DescribeTasksResponse/)
  ...
  function renderSomeType(type: TypeDoc.SomeType): string {
    ...
    if (type.type === "reference" && type.package === "@neo/platform") {
      return renderSstComponentType(type);
    }
    if (type.type === "reference" && type.package === "neo") {
  ...
  function renderSstComponentType(type: TypeDoc.ReferenceType) {
    ...
    if (dnsProvider) {
      return `[<code class="type">neo.${dnsProvider}.dns</code>](/docs/component/${dnsProvider}/dns/)`;
    }
    ...
    delete type._project;
    console.error(type);
    throw new Error(`Unsupported NEO component type`);
  }
  function renderSstSdkType(type: TypeDoc.ReferenceType) {
    ...
    delete type._project;
    console.error(type);
    throw new Error(`Unsupported NEO SDK type`);
  }
...
function renderCloudflareBindings(module: TypeDoc.DeclarationReflection) {
  ...
  // Get `getNEOLink().include` type
  const include = returnType.declaration.children?.find(
    (c) => c.name === "include"
  );
  if (!include) return lines;

  // Filter out `getNEOLink().include[].type` is `cloudflare.binding`
...
function useClassProviderNamespace(module: TypeDoc.DeclarationReflection) {
  ...
  //     "line": 280,
  //     "character": 13,
  //     "url": "https://github.com/neo/ion/blob/0776cea/platform/src/components/aws/astro.ts#L280"
  //   }
  // ],
  ...
  return namespace === "components" ? "neo" : `neo.${namespace}`;
}
...
async function buildExamples() {
  ...
  const app = await TypeDoc.Application.bootstrap({
    ...
    entryPoints: ["../examples/*/neo.config.ts"],
