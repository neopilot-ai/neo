...
declare var NEO_ASSET_MANIFEST: Record<string, string>;
declare var NEO_ROUTES: { regex: string; origin: "assets" | "server" }[];
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const route = NEO_ROUTES.find((r) => new RegExp(r.regex).test(pathname));
    ...
    async function lookupCache() {
      ...
      if (r.headers.get("etag") !== NEO_ASSET_MANIFEST[pathname]) return;
    ...
    async function respond(status: number, object: any) {
      ...
      if (NEO_ASSET_MANIFEST[pathname]) {
        headers.set("etag", NEO_ASSET_MANIFEST[pathname]);
