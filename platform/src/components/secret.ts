...
export class SecretMissingError extends VisibleError {
  constructor(public readonly secretName: string) {
    super(
      `Set a value for ${secretName} with \`neo secret set ${secretName} <value>\``,
    );
  }
}
 ...
 * then decrypted synchronously when your function starts up by the NEO SDK.
 *
 * @example
 *
 * #### Create a secret
 ...
 *
 * ```ts title="neo.config.ts"
 * const secret = new neo.Secret("MySecret", "my-secret-placeholder-value");
 * ```
 *
 ...
 *
 * ```sh title="Terminal"
 * neo secret set MySecret my-fallback-value --fallback
 * ```
 *
...
export class Secret extends Component implements Link.Linkable {
  ...
  constructor(name: string, placeholder?: Input<string>) {
    ...
    this._value = output(
      process.env["NEO_SECRET_" + this._name] ?? this._placeholder,
    ).apply((value) => {
