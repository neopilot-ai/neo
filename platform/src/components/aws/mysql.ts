...
export interface MysqlArgs {
   ...
   *
   * By default, your MySQL database is deployed in `neo dev`. But if you want to instead
 ...
 * const database = new neo.aws.Mysql("MyDatabase", { vpc });
 * ```
 *
 * #### Link to a resource
 *
 ...
 * #### Running locally
 *
 * By default, your RDS MySQL database is deployed in `neo dev`. But let's say you are
 ...
 *   --rm \
 *   -p 3306:3306 \
 *   -v $(pwd)/.neo/storage/mysql:/var/lib/mysql/data \
  ...
  constructor(name: string, args: MysqlArgs, opts?: ComponentResourceOptions) {
    ...
    function reference() {
      ...
      const input = instance.tags.apply((tags) => {
        return {
          ...
          passwordTag: tags?.["neo:ref:password"],
        };
      });
    ...
    function registerDev() {
      ...
      new DevCommand(`${name}Dev`, {
        ...
        environment: {
          NEO_DEV_COMMAND_MESSAGE: interpolate`Make sure your local MySQL server is using:
...
Listening on "${dev.host}:${dev.port}"...`,
   ...
   *   ? neo.aws.Mysql.get("MyDatabase", {
   *       id: "app-dev-mydatabase",
   *       proxyId: "app-dev-mydatabase-proxy"
   *     })
   *   : new neo.aws.Mysql("MyDatabase", {
   *       proxy: true
   *     });
   * ```
   *
...
const __pulumiType = "neo:aws:Mysql";
// @ts-expect-error
