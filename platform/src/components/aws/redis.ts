...
export interface RedisArgs {
   ...
   *
   * By default, your Redis instance is deployed in `neo dev`. But if you want to instead
   * connect to a locally running Redis server, you can configure the `dev` prop.
   *
   * :::note
   * By default, this creates a new Redis ElastiCache instance even in `neo dev`.
   * :::
   *
  ...
  constructor(name: string, args: RedisArgs, opts?: ComponentResourceOptions) {
    ...
    function reference() {
      ...
      const input = cluster.tags.apply((tags) => {
        ...
        if (!tags?.["neo:ref:secret"])
          throw new VisibleError(
            `Failed to lookup secret for Redis cluster "${name}".`,
          );

        return {
          secretRef: tags?.["neo:ref:secret"],
        };
      });
...
Listening on "${dev.host}:${dev.port}"...`,
   ...
   *   ? neo.aws.Redis.get("MyRedis", "app-dev-myredis")
   *   : new neo.aws.Redis("MyRedis");
   * ```
   *
