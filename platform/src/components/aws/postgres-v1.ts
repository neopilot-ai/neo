 ...
 *
 * For existing usage, rename `neo.aws.Postgres` to `neo.aws.Postgres.v1`. For new Postgres, use
 ...
 * Data API. This allowed your machine to connect to the database during "neo dev" without
 * the need for a VPN.
 * - In the new version, the database now uses AWS RDS Postgres. The "neo.aws.Vpc" component
 ...
 * const database = new neo.aws.Postgres.v1("MyDatabase", { vpc });
 * ```
 *
 * #### Change the scaling config
 *
 * ```js title="neo.config.ts"
 * new neo.aws.Postgres.v1("MyDatabase", {
 *   scaling: {
 *     min: "2 ACU",
 *     max: "128 ACU"
 *   },
...
export class Postgres extends Component implements Link.Linkable {
   ...
   *   ? neo.aws.Postgres.v1.get("MyDatabase", "app-dev-mydatabase")
   *   : new neo.aws.Postgres.v1("MyDatabase");
   * ```
   *
