...
export interface ClusterServiceArgs {
   ...
   *
   * :::note
   * In `neo dev` your service is run locally; it's not deployed.
   * :::
   *
   ...
   *
   * Prior to building the image, NEO will automatically add the `.neo` directory
  ...
  image?: Input<{
    /**
     * The path to the [Docker build context](https://docs.docker.com/build/building/context/#local-context). The path is relative to your project's `neo.config.ts`.
     * @default `"."`
     * @example
     *
 ...
 *
 * For existing usage, rename `neo.aws.Cluster` to `neo.aws.Cluster.v1`. For new Clusters, use
 ...
 * const cluster = new neo.aws.Cluster.v1("MyCluster", { vpc });
 * ```
 *
 * #### Add a service
 *
...
const __pulumiType = "neo:aws:Cluster";
// @ts-expect-error
