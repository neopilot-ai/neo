...
interface BucketLifecycleArgs {
   ...
   *
   * If not provided, NEO will generate a unique ID based on the bucket component name and rule index.
   *
   * @example
...
export class Bucket extends Component implements Link.Linkable {
  ...
  constructor(
    ...
    // the policy created here is created first. And NEO will throw an error if
   ...
   *   ? neo.aws.Bucket.get("MyBucket", "app-dev-mybucket-12345678")
   *   : new neo.aws.Bucket("MyBucket");
   * ```
   *
   ...
   *
   * ```js title="neo.config.ts" "link: [bucket]"
   * bucket.subscribe({
   *   handler: "src/subscriber.handler",
   *   link: [bucket]
   * }, {
   ...
   *
   * ```js title="neo.config.ts"
   * neo.aws.Bucket.subscribe(bucketArn, "src/subscriber.handler");
   * ```
   *
   ...
   *
   * ```js title="neo.config.ts"
   * neo.aws.Bucket.subscribe(bucketArn, {
   *   handler: "src/subscriber.handler",
   *   timeout: "60 seconds",
   * });
   * ```
   ...
   *
   * ```js title="neo.config.ts"
   * neo.aws.Bucket.subscribeQueue(bucketArn, queueArn);
   * ```
   *
   * Subscribe to specific S3 events.
   *
   ...
   *
   * ```js title="neo.config.ts"
   * neo.aws.Bucket.subscribe(bucketArn, topicArn);
   * ```
   *
   * Subscribe to specific S3 events.
   *
...
const __pulumiType = "neo:aws:Bucket";
// @ts-expect-error
