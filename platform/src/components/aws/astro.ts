...
export interface AstroArgs extends SsrSiteArgs {
   ...
   *
   * :::note
   * In `neo dev` your Astro site is run in dev mode; it's not deployed.
   * :::
   *
  ...
  /**
   * Path to the directory where your Astro site is located.  This path is relative to your `neo.config.ts`.
   *
   * By default it assumes your Astro site is in the root of your NEO app.
   * @default `"."`
   *
   * @example
   *
   ...
   * 2. Locally while running `astro dev` through `neo dev`.
   *
   * :::tip
...
export class Astro extends SsrSite {
  ...
  protected buildPlan(outputPath: Output<string>): Output<Plan> {
    return outputPath.apply((outputPath) => {
      const BUILD_META_FILE_NAME = "neo.buildMeta.json";
      const filePath = path.join(outputPath, "dist", BUILD_META_FILE_NAME);
      if (!fs.existsSync(filePath)) {
        throw new VisibleError(
          `Build metadata file not found at "${filePath}". Update your "astro-neo" adapter and rebuild your Astro site.`,
        );
      }
      ...
      if (
        ...
        throw new VisibleError(
          `Incompatible "astro-neo" adapter version detected. The Astro component requires "astro-neo" adapter version 3.1.2 or later.`,
        );
      }
...
const __pulumiType = "neo:aws:Astro";
// @ts-expect-error
