...
export interface RedisArgs {
   ...
   *
   * By default, your Redis cluster is deployed in `neo dev`. But if you want to instead
   * connect to a locally running Redis server, you can configure the `dev` prop.
   *
   * :::note
   * By default, this creates a new Redis ElastiCache cluster even in `neo dev`.
   * :::
   *
   ...
   * Redis instance both in `neo dev` and `neo deploy`.
   *
   * ```ts
   * {
   *   dev: {
 ...
 *
 * For existing usage, rename `neo.aws.Redis` to `neo.aws.Redis.v1`. For new Redis, use
 ...
 * const redis = new neo.aws.Redis.v1("MyRedis", { vpc });
 * ```
 *
 * #### Link to a resource
 *
 ...
 * #### Running locally
 *
 * By default, your Redis cluster is deployed in `neo dev`. But let's say you are running Redis
 * locally.
 *
 * ```bash
 * docker run \
 ...
 *
 * ```ts title="neo.config.ts" {3-6}
 * const redis = new neo.aws.Redis.v1("MyRedis", {
 *   vpc,
 *   dev: {
 *     host: "localhost",
 *     port: 6379
  ...
  constructor(name: string, args: RedisArgs, opts?: ComponentResourceOptions) {
    ...
    function registerDev() {
      ...
      new DevCommand(`${name}Dev`, {
        ...
        environment: {
          NEO_DEV_COMMAND_MESSAGE: interpolate`Make sure your local Redis server is using:
...
Listening on "${dev.host}:${dev.port}"...`,
   ...
   *   ? neo.aws.Redis.v1.get("MyRedis", "app-dev-myredis")
   *   : new neo.aws.Redis.v1("MyRedis");
   * ```
   *
  ...
  public static get(
    ...
    const secret = cluster.tags.apply((tags) =>
      tags?.["neo:auth-token-ref"]
        ? secretsmanager.getSecretVersionOutput(
            {
              secretId: tags["neo:auth-token-ref"],
            },
            opts,
          )
        : output(undefined),
...
const __pulumiType = "neo:aws:Redis";
// @ts-expect-error
