...
export interface EmailArgs {
   ...
   *
   * :::tip
   * NEO can automatically verify the domain for the `dns` adapter that's specified.
   * :::
   *
 ...
 *         Body: { Text: { Data: "Sent from my NEO app." } }
 *       }
 *     }
 *   })
 * );
...
export class Email extends Component implements Link.Linkable {
   ...
   *   ? neo.aws.Email.get("MyEmail", "spongebob@example.com")
   *   : new neo.aws.Email("MyEmail", {
   *       sender: "spongebob@example.com",
   *     });
   * ```
...
const __pulumiType = "neo:aws:Email";
// @ts-expect-error
