...
interface ServiceContainerArgs extends FargateContainerArgs {
  ...
  /**
   * Configure how this container works in `neo dev`. Same as the top-level
   * [`dev`](#dev).
   */
  dev?: {
    /**
     * The command that `neo dev` runs to start this in dev mode. Same as the top-level
     * [`dev.command`](#dev-command).
     */
    command: Input<string>;
    /**
     * Configure if you want to automatically start this when `neo dev` starts. Same as the
...
export interface ServiceArgs extends FargateBaseArgs {
   ...
   *
   * :::note
   * In `neo dev` your service is not deployed.
   * :::
   *
   * By default, your service in not deployed in `neo dev`. Instead, you can set the
   * `dev.command` and it'll be started locally in a separate tab in the
   * `neo dev` multiplexer. Read more about [`neo dev`](/docs/reference/cli/#dev).
   *
   * This makes it so that the container doesn't have to be redeployed on every change. To
   * disable this and deploy your service in `neo dev`, pass in `false`.
   */
  dev?:
    | false
    | {
         ...
         *
         * Since this component is not deployed in `neo dev`, there is no real URL. But if you are
        ...
        url?: Input<string>;
        /**
         * The command that `neo dev` runs to start this in dev mode. This is the command you run
  ...
  /**
   * Configure if `neo deploy` should wait for the service to be stable.
   *
   * :::tip
            ...
            ...transform(
              ...
              `${name}Service`,
              {
                ...
                networkConfiguration: {
                  // If the vpc is an NEO vpc, services are automatically deployed to the public
...
const __pulumiType = "neo:aws:Service";
// @ts-expect-error
