...
export interface StaticSiteArgs extends BaseStaticSiteArgs {
   ...
   *
   * :::note
   * In `neo dev` your static site is run in dev mode; it's not deployed.
   * :::
   *
  ...
  /**
   * Path to the directory where your static site is located. By default this assumes your static site is in the root of your NEO app.
   *
   * This directory will be uploaded to S3. The path is relative to your `neo.config.ts`.
   *
   * :::note
 ...
 * #### Running locally
 *
 * In `neo dev`, we don't deploy your site to AWS because we assume you are running it locally.
 *
 * :::note
 * Your static site will not be deployed when run locally with `neo dev`.
 * :::
 *
...
const __pulumiType = "neo:aws:StaticSite";
// @ts-expect-error
