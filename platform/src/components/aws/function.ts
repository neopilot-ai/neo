...
export interface FunctionArgs {
  /**
   * Disable running this function [Live](/docs/live/) in `neo dev`.
  ...
  live?: Input<false>;
  /**
   * Disable running this function [_Live_](/docs/live/) in `neo dev`.
   *
   * By default, the functions in your app are run locally in `neo dev`. To do this, a _stub_
   * version of your function is deployed, instead of the real function.
   *
   * :::note
   * In `neo dev` a _stub_ version of your function is deployed.
   * :::
   *
   ...
   *
   * Read more about [Live](/docs/live/) and [`neo dev`](/docs/reference/cli/#dev).
   *
   * @default `true`
   * @example
   * ```js
   ...
   * ```
   *
   * To make sure that the right runtime is used in `neo dev`, make sure to set the
   ...
   *
   * :::note
   * Streaming is currently not supported in `neo dev`.
   * :::
   *
   * While `neo dev` doesn't support streaming, you can use the
  ...
  url?: Input<
    ...
    | {
       ...
       * const router = new neo.aws.Router("Router", {
       *   domain: "example.com"
       * });
       * ```
       *
   ...
   *
   * By default, NEO will bundle your function
  ...
  nodejs?: Input<{
     ...
     * ```
     *
     * The path is relative to the location of the `neo.config.ts`.
     *
     ...
     * However, they are always generated during `neo dev`.
     *
     * :::tip[NEO Console]
  ...
  python?: Input<{
     ...
     * ```
     *
     * When you run `neo deploy`, it uses a built-in Dockerfile. It also needs
  ...
  copyFiles?: Input<
    {
      /**
       * Source path relative to the `neo.config.ts`.
       */
      from: Input<string>;
      /**
   ...
   * :::
   *
   * These are only added when the function is deployed. In `neo dev`, your functions are run
   ...
   * const fileSystem = new neo.aws.Efs("MyFileSystem", { vpc });
   * ```
   *
   * And pass it in.
   *
   ...
   *
   * ```js title="neo.config.ts"
   * const myVpc = new neo.aws.Vpc("MyVpc");
   * ```
   *
   * Or reference an existing VPC.
   *
   * ```js title="neo.config.ts"
   * const myVpc = neo.aws.Vpc.get("MyVpc", "vpc-12345678901234567");
   * ```
   *
   * And pass it in.
   *
 ...
 *   ```toml title="functions/pyproject.toml"
 *   [tool.uv.sources]
 *   neo = { git = "https://github.com/neopilot-ai/neo.git", subdirectory = "sdk/python", branch = "dev" }
 *   ```
 *   </TabItem>
 *   <TabItem label="Go">
 ...
 * #### Bundling
 *
 * Customize how NEO uses [esbuild](https://esbuild.github.io/) to bundle your Node.js
 * functions with the `nodejs` property.
 *
 * ```ts title="neo.config.ts" {3-5}
 * new neo.aws.Function("MyFunction", {
 *   handler: "src/lambda.handler",
 *   nodejs: {
 *     install: ["pg"]
...
export class Function extends Component implements Link.Linkable {
  ...
  constructor(
    ...
    function normalizeEnvironment() {
      return all([
        ...
        result.NEO_RESOURCE_App = JSON.stringify({
          name: $app.name,
          stage: $app.stage,
        });
        for (const linkable of link || []) {
          if (!Link.isLinkable(linkable)) continue;
          const def = linkable.getNEOLink();
          for (const item of def.include || []) {
            if (item.type === "environment") Object.assign(result, item.env);
          }
        }
        result.NEO_KEY = key;
        result.NEO_KEY_FILE = "resource.enc";
        if (dev) {
          const appsync = await Function.appsync();
          result.NEO_REGION = process.env.NEO_AWS_REGION!;
          result.NEO_APPSYNC_HTTP = appsync.http;
          result.NEO_APPSYNC_REALTIME = appsync.realtime;
          result.NEO_FUNCTION_ID = name;
          result.NEO_APP = $app.name;
          result.NEO_STAGE = $app.stage;
          result.NEO_ASSET_BUCKET = bootstrap.asset;
          if (process.env.NEO_FUNCTION_TIMEOUT) {
            result.NEO_FUNCTION_TIMEOUT = process.env.NEO_FUNCTION_TIMEOUT;
          }
        }
        return result;
      });
   ...
   *
   * ```ts title="neo.config.ts"
   * const fn = new neo.aws.Function("MyFunction", {
...
const __pulumiType = "neo:aws:Function";
// @ts-expect-error
