 ...
 *
 * ```ts title="neo.config.ts"
 * new neo.aws.CognitoUserPool("MyUserPool", {
 *   usernames: ["email"]
 * });
 * ```
 *
 ...
 *
 * ```ts title="neo.config.ts"
 * const GoogleClientId = new neo.Secret("GOOGLE_CLIENT_ID");
 * const GoogleClientSecret = new neo.Secret("GOOGLE_CLIENT_SECRET");
 *
...
export class CognitoUserPool extends Component implements Link.Linkable {
  ...
  public addClient(name: string, args?: CognitoUserPoolClientArgs) {
    ...
    // neo.aws.UserPool.getClient("pool", { userPooldID, clientID });
   ...
   *
   * ```ts title="neo.config.ts"
   * const GithubClientId = new neo.Secret("GITHUB_CLIENT_ID");
   * const GithubClientSecret = new neo.Secret("GITHUB_CLIENT_SECRET");
   *
   ...
   *   ? neo.aws.CognitoUserPool.get("MyUserPool", "us-east-1_gcF5PjhQK")
   *   : new neo.aws.CognitoUserPool("MyUserPool");
   * ```
   *
...
const __pulumiType = "neo:aws:CognitoUserPool";
// @ts-expect-error
