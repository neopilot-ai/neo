...
export interface CronArgs {
   ...
   * ```ts
   * {
   *   job: "arn:aws:lambda:us-east-1:000000000000:function:my-neo-app-jayair-MyFunction",
   * }
   * ```
   */
   ...
   * ```ts
   * {
   *   function: "arn:aws:lambda:us-east-1:000000000000:function:my-neo-app-jayair-MyFunction",
   * }
   * ```
   */
   ...
   * ```
   *
   * For ECS Fargate tasks, the event will be passed to the task as the `NEO_EVENT`
   * environment variable.
   *
   * ```ts
   * const event = JSON.parse(process.env.NEO_EVENT);
   * console.log(event.foo);
   * ```
   */
   ...
   *
   * :::note
   * The cron job continues to run even after you exit `neo dev`.
   * :::
   *
   * @example
   *
...
const __pulumiType = "neo:aws:Cron";
// @ts-expect-error
