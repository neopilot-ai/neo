...
export interface FargateBaseArgs {
   ...
   * const myCluster = new neo.aws.Cluster("MyCluster", { vpc });
   * ```
   *
   * And pass it in.
   *
   ...
   * const bucket = new neo.aws.Bucket("EnvBucket");
  ...
  logging?: Input<{
     ...
     * @default `"/neo/cluster/${CLUSTER_NAME}/${SERVICE_NAME}/${CONTAINER_NAME}"`
     */
    name?: Input<string>;
  }>;
  /**
        ...
        return all([v.logging, args.cluster.nodes.cluster.name]).apply(
          ([logging, clusterName]) => ({
            ...
            name:
              ...
              `/neo/cluster/${clusterName}/${physicalName(64, name)}/${v.name}`,
          }),
        );
      }
    }),
