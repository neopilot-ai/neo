...
export interface TaskArgs extends FargateBaseArgs {
   ...
   *
   * Defaults:
   * - If an NEO VPC component is passed to the `vpc` property, tasks run in public subnets
   * by default and `publicIp` defaults to `true`.
   * - If a non-NEO VPC is used, tasks run in the specified subnets and `publicIp` defaults
   * to `false`.
   *
   * @example
   * ```ts
   ...
   *
   * :::note
   * In `neo dev` a _stub_ version of your task is deployed.
   * :::
   *
   * By default, your task in not deployed in `neo dev`. Instead, you can set the `dev.command`
   * and it'll run locally in a **Tasks** tab in the `neo dev` multiplexer.
   *
   ...
   *
   * To disable this and deploy your task in `neo dev`, pass in `false`. Read more about
   * [Live](/docs/live/) and [`neo dev`](/docs/reference/cli/#dev).
   */
  dev?:
    | false
    | {
        /**
         * The command that `neo dev` runs in dev mode.
         */
 ...
 *
 * ```ts title="neo.config.ts"
 * const task = new neo.aws.Task("MyTask", { cluster });
 * ```
 *
            ...
            return [
              {
                ...v[0],
                image: output(
                  "ghcr.io/anomalyco/neo/bridge-task:20241224005724",
                ),
                environment: {
                  ...v[0].environment,
                  NEO_TASK_ID: name,
                  NEO_REGION: process.env.NEO_AWS_REGION!,
                  NEO_APPSYNC_HTTP: appsync.http,
                  NEO_APPSYNC_REALTIME: appsync.realtime,
                  NEO_APP: $app.name,
                  NEO_STAGE: $app.stage,
                },
              },
...
const __pulumiType = "neo:aws:Task";
// @ts-expect-error
