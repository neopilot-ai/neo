...
export class Dynamo extends Component implements Link.Linkable {
   ...
   *
   * ```js title="neo.config.ts"
   * neo.aws.Dynamo.subscribe("MySubscriber", streamArn, "src/subscriber.handler");
   * ```
   *
   * Add a filter to the subscription.
   *
   ...
   *
   * ```js title="neo.config.ts"
   * neo.aws.Dynamo.subscribe("MySubscriber", streamArn, {
   *   handler: "src/subscriber.handler",
   *   timeout: "60 seconds"
   * });
   * ```
   ...
   *  ? neo.aws.Dynamo.get("MyTable", "app-dev-mytable")
   *  : new neo.aws.Dynamo("MyTable");
   * ```
   *
...
const __pulumiType = "neo:aws:Dynamo";
// @ts-expect-error
