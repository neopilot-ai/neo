...
export interface TanStackStartArgs extends SsrSiteArgs {
  ...
  /**
   * Path to the directory where your TanStack Start app is located.  This path is relative to your `neo.config.ts`.
   *
   * By default it assumes your TanStack Start app is in the root of your NEO app.
   * @default `"."`
   *
   * @example
   *
   ...
   * 2. Locally while running `neo dev vinxi dev`.
   *
   * :::tip
 ...
 * new neo.aws.TanStackStart("MyWeb");
 * ```
 *
 * #### Change the path
 *
...
const __pulumiType = "neo:aws:TanstackStart";
// @ts-expect-error
