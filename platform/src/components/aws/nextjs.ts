...
export interface NextjsArgs extends SsrSiteArgs {
  /**
   * Configure how this component works in `neo dev`.
   *
   * :::note
   * In `neo dev` your Next.js app is run in dev mode; it's not deployed.
   * :::
   *
   * Instead of deploying your Next.js app, this starts it in dev mode. It's run
   * as a separate process in the `neo dev` multiplexer. Read more about
   * [`neo dev`](/docs/reference/cli/#dev).
   *
  ...
  /**
   * Path to the directory where your Next.js app is located. This path is relative to your `neo.config.ts`.
   *
   * By default this assumes your Next.js app is in the root of your NEO app.
   * @default `"."`
   *
   * @example
   *
   ...
   * 2. Locally while running through `neo dev`.
   *
   * :::tip
   ...
   *
   * :::note
   * This does not automatically update to the latest OpenNext version. It remains pinned to the version of NEO you have.
   * :::
   *
   * By default, this is pinned to the version of OpenNext that was released with the NEO version you are using. You can [find this in the source](https://github.com/neopilot-ai/neo/blob/dev/platform/src/components/aws/nextjs.ts#L30) under `DEFAULT_OPEN_NEXT_VERSION`.
   * OpenNext changed its package name from `open-next` to `@opennextjs/aws` in version `3.1.4`. NEO will choose the correct one based on the version you provide.
   *
   * @default The latest version of OpenNext pinned to the version of NEO you are using.
   * @example
   * ```js
   * {
        ...
        const plan = all([
          ...
          ([
            ...
            server: {
              ...
              injections: [
                [
                  `outer:if (process.env.NEO_KEY_FILE) {`,
                  `  const { readFileSync } = await import("fs")`,
                  `  const { createDecipheriv } = await import("crypto")`,
                  `  const key = Buffer.from(process.env.NEO_KEY, "base64");`,
                  `  const encryptedData = readFileSync(process.env.NEO_KEY_FILE);`,
                  ...
                  `  globalThis.NEO_KEY_FILE_DATA = decryptedData;`,
                  `}`,
                ].join("\n"),
              ],
            },
...
const __pulumiType = "neo:aws:Nextjs";
// @ts-expect-error
