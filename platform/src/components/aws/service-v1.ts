        ...
        return args.vpc.nodes.natGateways.apply((natGateways) => {
          if (natGateways.length === 0)
            throw new VisibleError(
              `The VPC configured for the service does not have NAT enabled. Enable NAT by configuring "nat" on the "neo.aws.Vpc" component.`,
            );
          return result;
        });
      }
    ...
    function createLogGroup() {
      return new cloudwatch.LogGroup(
        ...transform(
          ...
          `${name}LogGroup`,
          {
            name: interpolate`/neo/cluster/${cluster.name}/${name}`,
