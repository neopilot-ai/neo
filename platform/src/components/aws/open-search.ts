...
export interface OpenSearchArgs {
   ...
   * ```js
   * {
   *   password: new neo.Secret("MyDomainPassword").value
   * }
   * ```
   */
   ...
   *
   * By default, your OpenSearch domain is deployed in `neo dev`. But if you want to
   * instead connect to a locally running OpenSearch, you can configure the `dev` prop.
   *
   * :::note
   * By default, this creates a new OpenSearch domain even in `neo dev`.
   * :::
   *
   ...
   * instance both in `neo dev` and `neo deploy`.
   *
   * ```ts
   * {
   *   dev: {
 ...
 * const search = new neo.aws.OpenSearch("MySearch");
 * ```
 *
 * #### Link to a resource
 *
 ...
 * #### Running locally
 *
 * By default, your OpenSearch domain is deployed in `neo dev`. But let's say you are
 ...
 *   --rm \
 *   -p 9200:9200 \
 *   -v $(pwd)/.neo/storage/opensearch:/usr/share/opensearch/data \
 ...
 * const opensearch = new neo.aws.OpenSearch("MyOpenSearch", {
 *   dev: {
 *     url: "http://localhost:9200",
 *     username: "admin",
 *     password: "^Passw0rd^"
  ...
  constructor(
    ...
    function reference() {
      ...
      const input = domain.tags.apply((tags) => {
        if (!tags?.["neo:ref:username"])
          throw new VisibleError(
            `Failed to get username for OpenSearch ${name}.`,
          );
        if (!tags?.["neo:ref:password"])
          throw new VisibleError(
            `Failed to get password for OpenSearch ${name}.`,
          );

        return {
          username: tags["neo:ref:username"],
          password: tags["neo:ref:password"],
        };
      });
    ...
    function registerDev() {
      ...
      new DevCommand(`${name}Dev`, {
        ...
        environment: {
          NEO_DEV_COMMAND_MESSAGE: interpolate`Make sure your local OpenSearch server is using:
...
Listening on "${dev.url}"...`,
   ...
   *   ? neo.aws.OpenSearch.get("MyOpenSearch", "app-dev-myopensearch-efsmkrbt")
   *   : new neo.aws.OpenSearch("MyOpenSearch");
   * ```
   *
...
const __pulumiType = "neo:aws:OpenSearch";
// @ts-expect-error
