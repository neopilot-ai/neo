...
export class Cluster extends Component {
  ...
  constructor(
    ...
    function reference() {
      ...
      const clusterValidated = cluster.tags.apply((tags) => {
        const refVersion = tags?.["neo:ref:version"]
          ? parseComponentVersion(tags["neo:ref:version"])
          : undefined;
    ...
    function createCluster() {
      return new ecs.Cluster(
        ...transform(
          ...
          {
            tags: {
              "neo:ref:version": `${_version.major}.${_version.minor}`,
            },
          },
          { parent: self },
        ),
   ...
   *
   * ```ts
   * new neo.aws.Service("MyService", { cluster, ...args });
   * ```
   *
   ...
   *
   * ```ts
   * new neo.aws.Task("MyTask", { cluster, ...args });
   * ```
   *
