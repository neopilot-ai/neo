...
export class Bus extends Component implements Link.Linkable {
   ...
   *
   * ```js title="neo.config.ts"
   * neo.aws.Bus.subscribe("MySubscription", busArn, "src/subscriber.handler");
   * ```
   *
   ...
   *
   * ```js
   * neo.aws.Bus.subscribe("MySubscription", busArn, {
   *   handler: "src/subscriber.handler",
   *   timeout: "60 seconds"
   * });
   * ```
   ...
   *
   * ```js title="neo.config.ts"
   * neo.aws.Bus.subscribeQueue("MySubscription", busArn, queue);
   * ```
   *
   ...
   *
   * ```js
   * neo.aws.Bus.subscribeQueue("MySubscription", busArn, "arn:aws:sqs:us-east-1:123456789012:my-queue");
   * ```
   */
   ...
   *   ? neo.aws.Bus.get("MyBus", "app-dev-MyBus")
   *   : new neo.aws.Bus("MyBus");
   * ```
   *
...
const __pulumiType = "neo:aws:Bus";
// @ts-expect-error
