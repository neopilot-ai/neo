...
interface InlineRouterBucketRouteArgs extends InlineBaseRouteArgs {
   ...
   * const myBucket = new neo.aws.Bucket("MyBucket", {
   *   access: "cloudfront"
   * });
   * ```
   *
...
export interface RouterArgs {
  ...
  invalidation?: Input<
    ...
    | {
        /**
         * Configure if `neo deploy` should wait for the CloudFront cache invalidation to finish.
         *
         * :::tip
 ...
 * new neo.aws.Router("MyRouter");
 * ```
 *
 * #### Add a custom domain
 *
 * ```ts {2} title="neo.config.ts"
 * new neo.aws.Router("MyRouter", {
 *   domain: "myapp.com"
 * });
 * ```
 ...
 *       }
 *     })
 *   : neo.aws.Router.get("MyRouter", "E1XWRGCYGTFB7Z");
 * ```
 *
 * #### Route to a URL
 *
 ...
 * #### Route to a frontend
 *
 * ```ts title="neo.config.ts" {4-6}
 * const router = new neo.aws.Router("MyRouter");
 *
 * const mySite = new neo.aws.Nextjs("MyWeb", {
 *   router: {
 *     instance: router
 *   }
 * });
 ...
 * #### Limits
 *
 * There are some limits on this setup but it's managed by NEO.
 *
  ...
  constructor(
    ...
    function reference() {
      ...
      const tags = cdn.nodes.distribution.tags.apply((tags) => {
        if (tags?.["neo:ref:version"] !== _refVersion.toString()) {
        ...
        return {
          kvStoreArn: tags?.["neo:ref:kv"],
          kvNamespace: tags?.["neo:ref:kv-namespace"],
          hasInlineRoutes: tags?.["neo:ref:kv"] === undefined,
        };
      });
      ...
      function createCachePolicy() {
        return new cloudfront.CachePolicy(
          ...transform(
            ...
            {
              comment: "NEO server response cache policy",
              defaultTtl: 0,
              maxTtl: 31536000, // 1 year
              minTtl: 0,
              parametersInCacheKeyAndForwardedToOrigin: {
...
async function handler(event) {
        ...
        return new Cdn(
          ...transform(
            ...
            `${name}Cdn`,
            {
              ...
              tags: {
                "neo:ref:kv": kvStoreArn,
                "neo:ref:kv-namespace": kvNamespace,
                "neo:ref:version": _refVersion.toString(),
              },
            },
            { parent: self },
          ),
   ...
   *   ? neo.aws.Router.get("MyRouter", "E2IDLMESRN6V62")
   *   : new neo.aws.Router("MyRouter");
   * ```
   *
...
const __pulumiType = "neo:aws:Router";
// @ts-expect-error
