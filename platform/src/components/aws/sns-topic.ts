...
export class SnsTopic extends Component implements Link.Linkable {
   ...
   *
   * ```js title="neo.config.ts"
   * neo.aws.SnsTopic.subscribe("MySubscriber", topicArn, "src/subscriber.handler");
   * ```
   *
   * Add a filter to the subscription.
   *
   ...
   *
   * ```js title="neo.config.ts"
   * neo.aws.SnsTopic.subscribe("MySubscriber", topicArn, {
   *   handler: "src/subscriber.handler",
   *   timeout: "60 seconds"
   * });
   * ```
   ...
   *
   * ```js title="neo.config.ts"
   * const queue = neo.aws.Queue("MyQueue");
   * ```
   *
   ...
   *
   * ```js title="neo.config.ts"
   * neo.aws.SnsTopic.subscribeQueue("MySubscriber", topicArn, queueArn);
   * ```
   *
   * Add a filter to the subscription.
   *
   ...
   *   ? neo.aws.SnsTopic.get("MyTopic", "arn:aws:sns:us-east-1:123456789012:MyTopic")
   *   : new neo.aws.SnsTopic("MyTopic");
   * ```
   *
...
const __pulumiType = "neo:aws:SnsTopic";
// @ts-expect-error
