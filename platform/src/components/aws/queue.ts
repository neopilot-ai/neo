...
export interface QueueArgs {
   ...
   * const deadLetterQueue = new neo.aws.Queue("MyDLQ");
   *
   * new neo.aws.Queue("MyQueue", {
...
export class Queue extends Component implements Link.Linkable {
   ...
   *
   * ```js title="neo.config.ts"
   * neo.aws.Queue.subscribe(queueArn, "src/subscriber.handler");
   * ```
   *
   * Add a filter to the subscription.
   *
   ...
   *
   * ```js title="neo.config.ts"
   * neo.aws.Queue.subscribe(queueArn, {
   *   handler: "src/subscriber.handler",
   *   timeout: "60 seconds"
   * });
   * ```
   ...
   *   ? neo.aws.Queue.get("MyQueue", "https://sqs.us-east-1.amazonaws.com/123456789012/MyQueue")
   *   : new neo.aws.Queue("MyQueue");
   * ```
   *
...
const __pulumiType = "neo:aws:Queue";
// @ts-expect-error
