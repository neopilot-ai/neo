...
export interface StepFunctionsArgs {
   ...
   * const foo = neo.aws.StepFunctions.pass({ name: "Foo" });
   * const bar = neo.aws.StepFunctions.succeed({ name: "Bar" });
   *
 ...
 *
 * :::note
 * This component is currently in beta. Please [report any issues](https://github.com/neopilot-ai/neo/issues) you find.
 * :::
 *
 ...
 *
 * ```ts title="neo.config.ts" {5-8,12}
 * const myFunction = new neo.aws.Function("MyFunction", {
 *   handler: "src/index.handler"
 * });
 *
 * const invoke = neo.aws.StepFunctions.lambdaInvoke({
 *   name: "InvokeMyFunction",
 *   function: myFunction
 * });
 * const done = neo.aws.StepFunctions.succeed({ name: "Done" });
 *
 * new neo.aws.StepFunctions("MyStateMachine", {
...
export class StepFunctions extends Component implements Link.Linkable {
   ...
   * @example
   * ```ts title="neo.config.ts"
   * const processPayment = neo.aws.StepFunctions.choice({ name: "ProcessPayment" });
   *
   * const makePayment = neo.aws.StepFunctions.lambdaInvoke({ name: "MakePayment" });
   * const sendReceipt = neo.aws.StepFunctions.lambdaInvoke({ name: "SendReceipt" });
   * const failure = neo.aws.StepFunctions.fail({ name: "Failure" });
   *
   ...
   * @example
   * ```ts title="neo.config.ts"
   * neo.aws.StepFunctions.fail({ name: "Failure" });
   * ```
   */
   ...
   * @example
   * ```ts title="neo.config.ts"
   * const processor = neo.aws.StepFunctions.lambdaInvoke({
   *   name: "Processor",
   *   function: "src/processor.handler"
   * });
   *
   * neo.aws.StepFunctions.map({
   *   processor,
   *   name: "Map",
   ...
   * @example
   * ```ts title="neo.config.ts"
   * const processorA = neo.aws.StepFunctions.lambdaInvoke({
   *   name: "ProcessorA",
   *   function: "src/processorA.handler"
   * });
   *
   * const processorB = neo.aws.StepFunctions.lambdaInvoke({
   *   name: "ProcessorB",
   ...
   * @example
   * ```ts title="neo.config.ts"
   * neo.aws.StepFunctions.succeed({ name: "Succeed" });
   * ```
   */
   ...
   *
   * - `neo.aws.StepFunctions.lambdaInvoke` to invoke a Lambda function.
   * - `neo.aws.StepFunctions.ecsRunTask` to run an ECS task.
   * - `neo.aws.StepFunctions.eventBridgePutEvents` to send custom events to
   *   EventBridge.
   *
   ...
   * @example
   * ```ts title="neo.config.ts"
   * neo.aws.StepFunctions.lambdaInvoke({
   *   name: "LambdaInvoke",
   ...
   *
   * ```ts title="neo.config.ts"
   * const myLambda = new neo.aws.Function("MyLambda", {
   *   handler: "src/index.handler"
   * });
   *
   * neo.aws.StepFunctions.lambdaInvoke({
   ...
   * @example
   * ```ts title="neo.config.ts"
   * const myTopic = new neo.aws.SnsTopic("MyTopic");
   *
   * neo.aws.StepFunctions.snsPublish({
   *   name: "SnsPublish",
   *   topic: myTopic,
   *   message: "Hello, world!"
   * });
   ...
   * @example
   * ```ts title="neo.config.ts"
   * const myQueue = new neo.aws.Queue("MyQueue");
   *
   * neo.aws.StepFunctions.sqsSendMessage({
   *   name: "SqsSendMessage",
   *   queue: myQueue,
   ...
   * @example
   * ```ts title="neo.config.ts"
   * const myCluster = new neo.aws.Cluster("MyCluster");
   * const myTask = new neo.aws.Task("MyTask", { cluster: myCluster });
   *
   * neo.aws.StepFunctions.ecsRunTask({
   *   name: "RunTask",
   *   task: myTask
   * });
   * ```
   ...
   * @example
   * ```ts title="neo.config.ts"
   * const myBus = new neo.aws.EventBus("MyBus");
   *
   * neo.aws.StepFunctions.eventBridgePutEvents({
...
const __pulumiType = "neo:aws:StepFunctions";
// @ts-expect-error
