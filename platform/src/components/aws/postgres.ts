...
export interface PostgresArgs {
   ...
   *
   * By default, your Postgres database is deployed in `neo dev`. But if you want to instead
   * connect to a locally running Postgres database, you can configure the `dev` prop.
   *
   * :::note
   * This will not create an RDS database in `neo dev`.
   * :::
   *
 ...
 * const database = new neo.aws.Postgres("MyDatabase", { vpc });
 * ```
 *
 * #### Link to a resource
 *
 ...
 * #### Running locally
 *
 * By default, your RDS Postgres database is deployed in `neo dev`. But let's say you are running
 * Postgres locally.
 *
 * ```bash
 * docker run \
  ...
  constructor(
    ...
    function reference() {
      ...
      const input = instance.tags.apply((tags) => {
        registerVersion(
          tags?.["neo:component-version"]
            ? parseInt(tags["neo:component-version"])
            : undefined,
        );

        return {
          proxyId: output(ref.proxyId),
          passwordTag: tags?.["neo:lookup:password"],
        };
      });
    ...
    function registerDev() {
      ...
      new DevCommand(`${name}Dev`, {
        ...
        environment: {
          NEO_DEV_COMMAND_MESSAGE: interpolate`Make sure your local PostgreSQL server is using:
...
Listening on "${dev.host}:${dev.port}"...`,
      ...
      return new rds.Instance(
        ...transform(
          ...
          `${name}Instance`,
          {
            ...
            tags: {
              "neo:component-version": _version.toString(),
              "neo:lookup:password": secret.id,
            },
          },
          {
            parent: self,
   ...
   *   ? neo.aws.Postgres.get("MyDatabase", {
   *       id: "app-dev-mydatabase",
   *       proxyId: "app-dev-mydatabase-proxy"
   *     })
   *   : new neo.aws.Postgres("MyDatabase", {
   *       proxy: true
   *     });
   * ```
   *
...
const __pulumiType = "neo:aws:Postgres";
// @ts-expect-error
