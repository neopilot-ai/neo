...
export interface AuroraArgs {
   ...
   * ```js
   * {
   *   password: (new neo.Secret("MyDBPassword")).value
   * }
   * ```
   */
   ...
   * You also doesn't need the `neo tunnel` or a VPN to connect to it from your local machine.
   *
   ...
   *
   * By default, your Aurora database is deployed in `neo dev`. But if you want to instead
   ...
   * database both in `neo dev` and `neo deploy`.
   *
   * ```ts
   * {
   *   dev: {
 ...
 * const database = new neo.aws.Aurora("MyDatabase", {
 *   engine: "postgres",
 *   vpc
 * });
 * ```
 ...
 * connection, and works over HTTP. You also don't need the `neo tunnel` or a VPN to connect
 ...
 * #### Running locally
 *
 * By default, your Aurora database is deployed in `neo dev`. But let's say you are running
 * Postgres locally.
 *
 * ```bash
 * docker run \
 *   --rm \
 *   -p 5432:5432 \
 *   -v $(pwd)/.neo/storage/postgres:/var/lib/postgresql/data \
 ...
 * ```
 *
 * You can connect to it in `neo dev` by configuring the `dev` prop.
 *
 * ```ts title="neo.config.ts" {4-9}
 * new neo.aws.Aurora("MyDatabase", {
 *   engine: "postgres",
 *   vpc,
 *   dev: {
  ...
  constructor(name: string, args: AuroraArgs, opts?: ComponentResourceOptions) {
    ...
    function reference() {
      ...
      const secretId = cluster.tags
        .apply((tags) => tags?.["neo:ref:password"])
        .apply((passwordTag) => {
          if (!passwordTag)
            throw new VisibleError(
    ...
    function registerDev() {
      ...
      new DevCommand(`${name}Dev`, {
        dev: {
          title: name,
          autostart: true,
          command: `neo print-and-not-quit`,
        },
        environment: {
          NEO_DEV_COMMAND_MESSAGE: interpolate`Make sure your local database is using:
...
Listening on "${dev.host}:${dev.port}"...`,
        ...
        ...transform(
          ...
          `${name}Cluster`,
          {
            ...
            tags: proxy.apply((proxy) => ({
              "neo:ref:password": secret.id,
              ...(proxy ? { "neo:ref:proxy": proxy.id } : {}),
            })),
          },
          {
            parent: self,
   ...
   *   ? neo.aws.Aurora.get("MyDatabase", "app-dev-mydatabase")
   *   : new neo.aws.Aurora("MyDatabase");
   * ```
   *
...
const __pulumiType = "neo:aws:Aurora";
// @ts-expect-error
