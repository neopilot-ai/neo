 ...
 * const stream = new neo.aws.KinesisStream("MyStream");
 * ```
 *
...
export class KinesisStream extends Component implements Link.Linkable {
   ...
   *
   * ```js title="neo.config.ts"
   * neo.aws.KinesisStream.subscribe("MySubscriber", streamArn, "src/subscriber.handler");
   * ```
   *
   * Add a filter to the subscription.
   *
   ...
   *
   * ```js title="neo.config.ts"
   * neo.aws.KinesisStream.subscribe("MySubscriber", streamArn, {
   *   handler: "src/subscriber.handler",
   *   timeout: "60 seconds"
   * });
   * ```
...
const __pulumiType = "neo:aws:KinesisStream";
// @ts-expect-error
