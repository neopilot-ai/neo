...
export interface VpcArgs {
   ...
   *
   * If you are running `neo dev`, a tunnel will be automatically created to the bastion host.
   * This uses a network interface to forward traffic from your local machine to the bastion host.
   *
   * You can learn more about [`neo tunnel`](/docs/reference/cli#tunnel).
   *
   * @default `false`
   * @example
   * ```ts
...
export class Vpc extends Component implements Link.Linkable {
    ...
    function reference() {
      ...
      const privateKeyValue = bastionInstance.apply((v) => {
        ...
        const param = ssm.Parameter.get(
          ...
          interpolate`/neo/vpc/${vpcId}/private-key-value`,
          undefined,
          { parent: self },
        );
        return param.value;
    ...
    function createKeyPair() {
      const ret = output(args.bastion).apply((bastion) => {
        ...
        new ssm.Parameter(
          `${name}PrivateKeyValue`,
          {
            name: interpolate`/neo/vpc/${vpc.id}/private-key-value`,
              ...
              ...transform(
                ...
                `${name}NatInstance${i + 1}`,
                {
                  ...
                  tags: {
                    ...
                    "neo:is-nat": "true",
                    ...(bastion && i === 0 ? { "neo:is-bastion": "true" } : {}),
                  },
                },
                { parent: self },
              ),
   ...
   *   ? neo.aws.Vpc.get("MyVPC", "vpc-0be8fa4de860618bb")
   *   : new neo.aws.Vpc("MyVPC");
   * ```
   *
...
const __pulumiType = "neo:aws:Vpc";
// @ts-expect-error
