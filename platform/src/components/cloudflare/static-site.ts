...
export interface StaticSiteArgs extends BaseStaticSiteArgs {
   ...
   *
   * This directory will be uploaded to KV. The path is relative to your `neo.config.ts`.
   *
   * :::note
        ...
        `${name}Router`,
        {
          ...
          build: {
            esbuild: assetManifest.apply((assetManifest) => ({
              define: {
                NEO_ASSET_MANIFEST: JSON.stringify(
...
const __pulumiType = "neo:cloudflare:StaticSite";
// @ts-expect-error
