 ...
 * new neo.cloudflare.Astro("MyWeb");
 * ```
 *
 * #### Change the path
 *
      ...
      if (!(await existsAsync(path.join(distPath, "_worker.js", "index.js")))) {
        throw new VisibleError(
          `SSR server bundle "_worker.js" not found in the build output at:\n` +
            ...
            `If your Astro project is entirely pre-rendered, use the \`neo.cloudflare.StaticSite\` component instead of \`neo.cloudflare.Astro\`.`,
        );
      }
...
const __pulumiType = "neo:cloudflare:Astro";
// @ts-expect-error
