...
export interface SolidStartArgs extends SsrSiteArgs {
   ...
   *
   * :::note
   * In `neo dev` your SolidStart app is run in dev mode; it's not deployed.
   * :::
   *
  ...
  dev?: SsrSiteArgs["dev"];
  /**
   * Path to the directory where your SolidStart app is located.  This path is relative to your `neo.config.ts`.
   *
   * By default it assumes your SolidStart app is in the root of your NEO app.
   * @default `"."`
   *
   * @example
   *
   ...
   * 2. Locally while running `vinxi dev` through `neo dev`.
   *
   * :::tip
 ...
 * new neo.cloudflare.SolidStart("MyWeb");
 * ```
 *
 * #### Change the path
 *
      ...
      if (
        ...
        throw new VisibleError(
          `SSR server bundle "index.mjs" not found in the build output at:\n` +
            ...
            `If your SolidStart project is entirely pre-rendered, use the \`neo.cloudflare.StaticSite\` component instead of \`neo.cloudflare.SolidStart\`.`,
        );
      }
...
const __pulumiType = "neo:cloudflare:SolidStart";
// @ts-expect-error
