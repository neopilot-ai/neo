/**
 * The `neo.config.ts` file is used to configure your NEO app and its resources.
 *
 * ```ts
 ...
 * function is where you define your resources using NEO or Pulumi's components.
 *
 ...
 *
 * :::caution
 * Do not `import` the provider packages in your `neo.config.ts`.
 * :::
 *
 * Since NEO manages importing your provider packages, it's recommended not to add any imports
 * in your `neo.config.ts`.
 *
 * ---
 *
 * #### .env
 ...
 * They need to be in the same directory as your `neo.config.ts`.
 *
 * ```bash title=".env"
 * MY_ENV_VAR=hello
 * ```
 ...
 *
 * :::note
 * You need to restart `neo dev` for changes in your `.env` files to take effect.
 * :::
 *
...
export interface App {
  /**
   * The version of NEO supported by the app. The CLI will fail any commands if the version does
   * not match.
   *
   * :::tip
   * Useful in CI where you don't want it to automatically deploy with a new version of NEO.
   * :::
   *
   * @default The latest version of NEO.
   *
   * @example
   *
   ...
   * ```
   *
   * This applies to not just the `neo remove` command but also cases where you remove a
   * resource from the `neo.config.ts` and run `neo dev` or `neo deploy`.
   *
   * <VideoAside title="Watch a video on how to protect your prod resources" href="https://youtu.be/fb6UBGwgDuA" />
   *
   * To control how a stage is handled on `neo remove`, check out the `protect` prop.
   *
   * @default `"retain"`
   */
   ...
   *
   * :::tip
   * You'll need to run `neo install` after you update the `providers` in your config.
   * :::
   *
  ...
  /**
   * The provider NEO will use to store the state for your app. The state keeps track of all your resources and secrets. The state is generated locally and backed up in your cloud provider.
   *
   *
   * Currently supports AWS, Cloudflare and local.
   *
   * :::tip
   * NEO uses the `home` provider to store the state for your app. If you use the local provider it will be saved on your machine. You can see where by running `neo version`.
   * :::
   *
  ...
  /**
   * If set to `true`, the `neo remove` CLI will not run and will error out.
   *
   * This is useful for preventing cases where you run `neo remove --stage <stage>` for the
   * wrong stage.
   *
   * :::tip
   ...
   * ```
   *
   * However, this only applies to `neo remove` for stages.
   *
   * <VideoAside title="Watch a video on how to protect your prod resources" href="https://youtu.be/fb6UBGwgDuA" />
   *
   * If you accidentally remove a resource from the `neo.config.ts` and run `neo deploy` or
   * `neo dev`, it'll still get removed. To avoid this, check out the `removal` prop.
   */
  protect?: boolean;

  /**
   * Configure which directories should be watched for changes when running `neo dev`.
...
export interface Config {
  ...
  /**
   * Configure how your app works with the NEO Console.
   */
  console?: {
    /**
     ...
     *         ? await $`pnpm neo remove`
     *         : await $`pnpm neo deploy`;
     *     }
     *   }
     * }
     * ```
    ...
    autodeploy: {
       ...
       * are using, installs the dependencies, and runs `neo deploy` or `neo remove`
       * based on the event.
       *
       ...
       *
       * :::caution
       * If you don't run `neo deploy`, your app won't be deployed.
       * :::
       *
       * This means that if you don't run `neo deploy`, your app won't be deployed.
       *
       * :::tip
       ...
       * ```
       *
       * You'll notice we are not passing in `--stage` to the NEO commands. This is because the `NEO_STAGE` environment variable is already set in
       * the build process.
       *
       * :::tip
       * You don't need to pass in `--stage` to the NEO commands.
       * :::
       *
   ...
   *
   * :::note
   * You can use NEO and Pulumi components only in the `run` function.
   * :::
   *
   ...
   * ```
   *
   * This will display the following in the CLI on `neo deploy` and `neo dev`.
   *
   * ```bash frame=\"none\"
   * bucket: bucket-jOaikGu4rla
   * ```
   *
   * These outputs are also written to a `.neo/outputs.json` file after every successful deploy.
   * It contains the above outputs in JSON.
   *
   * ```json title=".neo/outputs.json"
   * {"bucket": "bucket-jOaikGu4rla"}
   * ```
   */
