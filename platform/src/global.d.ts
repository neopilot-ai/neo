/**
 * The Global library is a collection of `$` functions and variables that are available in the `run` function, of your [`neo.config.ts`](/docs/reference/config/).
 *
 * You don't need to import the Global library. It's available in the `run` function of your `neo.config.ts`.
 *
 * :::note
 * The Global library is only available in the `run` function of your `neo.config.ts`.
 * :::
 *
...
import * as _neo from "./components/index";
...
interface $APP
  extends Readonly<{
    ...
    stage: string;
    /**
     * The removal policy for the current stage. If `removal` was not set in the `neo.config.ts`, this will be return its default value, `retain`.
     */
    ...
    /**
     * If true, prevents `neo remove` from being executed on this stage
     */
    protect: boolean;
  }> {}
...
declare global {
   ...
   * ```ts title="neo.config.ts"
   * const bucket1 = new neo.aws.Bucket("MyBucket1");
   * const bucket2 = new neo.aws.Bucket("MyBucket2");
   * ```
   *
   ...
   *
   * ```ts title="neo.config.ts"
   * $transform(neo.aws.Function, (args, opts, name) => {
   ...
   *
   * ```ts title="neo.config.ts"
   * new neo.aws.Function(name: string, args: FunctionArgs, opts?: pulumi.ComponentResourceOptions)
   * ```
   */
  ...
  /**
   * Returns `true` if the app is running in `neo dev`.
   */
