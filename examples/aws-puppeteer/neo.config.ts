 ...
 * 2. Chromium
 *    - In `neo dev`, we'll use a locally installed Chromium version.
 *    - In `neo deploy`, we'll use the [`@sparticuz/chromium`](https://github.com/sparticuz/chromium) package. It comes with a pre-built binary for Lambda.
 *
 * #### Chromium version
 *
 ...
 * ```
 *
 * You'll notice we are using the right binary with the `NEO_DEV` environment variable.
 *
 * ```ts title="index.ts" {4-6}
 * const browser = await puppeteer.launch({
 *   args: chromium.args,
 *   defaultViewport: chromium.defaultViewport,
 *   executablePath: process.env.NEO_DEV
 ...
 * ```
 *
 * And on deploy, NEO will use the right binary.
 *
 * :::tip
