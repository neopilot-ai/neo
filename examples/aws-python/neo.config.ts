 ...
 * ## AWS Lambda Python
 *
 * NEO uses [uv](https://docs.astral.sh/uv/) to manage your Python runtime, make
 ...
 * package can be built and deployed as a Lambda function using NEO. Drop-in mode
 ...
 * new neo.aws.Function("MyPythonFunction", {
 ...
 * ```
 *
 * NEO will traverse up from the handler path and look for the nearest
 * `pyproject.toml`. And will throw an error if it can't find one.
 *
 * To access linked resources, you can use the NEO SDK.
 *
 ...
 * ```
 *
 * This makes sure that your functions work the same in `neo dev` as `neo deploy`.
 */
