 ...
 * with the [`neo shell`](/docs/reference/cli) CLI.
 *
 * ```json title="package.json"
 * "db:generate": "neo shell drizzle-kit generate",
 * "db:migrate": "neo shell drizzle-kit migrate",
 * "db:push": "neo shell drizzle-kit push",
 * "db:studio": "neo shell drizzle-kit studio",
 * ```
 *
 ...
 *
 * ```bash
 * npx neo shell --stage production npx drizzle-kit push
 * ```
 *
 * If you are running this locally, you'll need to have a tunnel running.
 *
 * ```bash
 * npx neo tunnel --stage production
 * ```
 *
