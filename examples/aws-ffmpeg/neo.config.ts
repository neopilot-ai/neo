 ...
 * ```
 *
 * We don't need a layer when we deploy this because NEO will use the right binary for the
 * target Lambda architecture; including `arm64`.
 *
 * ```json title="neo.config.ts"
 * {
...
export default $config({
  ...
  async run() {
    const func = new neo.aws.Function("MyFunction", {
      url: true,
      memory: "2 GB",
      timeout: "15 minutes",
