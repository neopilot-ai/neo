 ...
 * ```
 *
 * The data is saved to the `.neo/storage` directory. So if you restart the dev server, the
 ...
 * ```
 *
 * By providing the `dev` prop for OpenSearch, NEO will use the local OpenSearch process and
 * not deploy a new OpenSearch domain when running `neo dev`.
 *
 ...
 * ```
 *
 * The above will work in both `neo dev` and `neo deploy`.
 */
