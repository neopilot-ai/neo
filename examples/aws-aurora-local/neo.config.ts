 ...
 * ```
 *
 * By providing the `dev` prop for Postgres, NEO will use the local Postgres instance and
 * not deploy a new RDS database when running `neo dev`.
 *
...
export default $config({
  ...
  async run() {
    ...
    const database = new neo.aws.Aurora("MyPostgres", {
      engine: "postgres",
      dev: {
        username: "postgres",
        password: "password",
