...
import (
	...
	"github.com/neopilot-ai/neo/v3/pkg/telemetry"
	"github.com/neopilot-ai/neo/v3/pkg/types"
	"golang.org/x/exp/slices"
	"golang.org/x/sync/errgroup"
)

func (p *Project) Run(ctx context.Context, input *StackInput) error {
	// if flag.NEO_EXPERIMENTAL {
...
func (p *Project) RunNext(ctx context.Context, input *StackInput) error {
	...
	outfile := filepath.Join(p.PathPlatformDir(), fmt.Sprintf("neo.config.%v.mjs", time.Now().UnixMilli()))
	os.WriteFile(
	...
	providerShim = append(providerShim, fmt.Sprintf("import * as neo from \"%s\";", path.Join(filepath.ToSlash(p.PathPlatformDir()), "src/components")))
	...
	if err != nil {
		...
		log.Error("failed to build neo.config.ts", "err", err)
		return err
	}
	log.Info("built neo.config.ts", "to", outfile)
	if !flag.NEO_NO_CLEANUP {
	...
	env := os.Environ()
	// Prepend NEO's bin directory to PATH to ensure NEO's Pulumi is used over system-installed versions
	for i, e := range env {
	...
	for key, value := range fallback {
		env = append(env, fmt.Sprintf("NEO_SECRET_%v=%v", key, value))
	}
	...
	if input.ServerPort != 0 {
		env = append(env, "NEO_SERVER=http://127.0.0.1:"+fmt.Sprint(input.ServerPort))
	}
	pulumiPath := global.PulumiPath()
	if flag.NEO_PULUMI_PATH != "" {
		pulumiPath = flag.NEO_PULUMI_PATH
	}
