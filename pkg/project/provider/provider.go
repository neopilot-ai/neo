...
const SSM_NAME_BOOTSTRAP = "/neo/bootstrap"

var ErrLockExists = fmt.Errorf("Concurrent update detected, run `neo unlock --stage=<stage>` to delete lock file and retry.")
...
func Passphrase(backend Home, app, stage string) (string, error) {
	...
	if passphrase == "" {
		...
		passphrase = flag.NEO_PASSPHRASE
		if passphrase == "" {
			bytes := make([]byte, 32)
...
func Lock(backend Home, version, command, app, stage string) (*Update, error) {
	...
	lockData.RunID = os.Getenv("NEO_RUN_ID")
...
func ForceUnlock(backend Home, version, app, stage string) error {
	...
	if lockData.UpdateID != "" {
		err = PutUpdate(backend, app, stage, &Update{
			...
			Errors: []SummaryError{
				{
					Message: "Update did not complete and was force unlocked with the `neo unlock` command",
				},
			},
		})
		if err != nil {
