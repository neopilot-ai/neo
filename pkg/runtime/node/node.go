...
func New(version string) *Runtime {
	weight := int64(4)
	if flag.NEO_BUILD_CONCURRENCY_FUNCTION != "" {
		weight, _ = strconv.ParseInt(flag.NEO_BUILD_CONCURRENCY_FUNCTION, 10, 64)
	} else if flag.NEO_BUILD_CONCURRENCY != "" {
		weight, _ = strconv.ParseInt(flag.NEO_BUILD_CONCURRENCY, 10, 64)
	}
