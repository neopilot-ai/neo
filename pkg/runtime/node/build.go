	...
	plugins := []esbuild.Plugin{
		{
			...
			Setup: func(build esbuild.PluginBuild) {
				...
				build.OnResolve(api.OnResolveOptions{Filter: `^sst$`}, func(args api.OnResolveArgs) (api.OnResolveResult, error) {
					...
					pkg := build.Resolve("neo", esbuild.ResolveOptions{
						ResolveDir: args.ResolveDir,
						Importer:   args.Importer,
						Kind:       args.Kind,
						With:       args.With,
						PluginName: "neo-version-check",
					...
					if pkg.Path != "" {
						...
						if r.version != "dev" && pkgjson.Version != r.version {
							return api.OnResolveResult{}, fmt.Errorf("The neo package your application is importing (%v) does not match the neo cli version (%v). Make sure the version of neo in package.json is correct across your entire repo.", pkgjson.Version, r.version)
						}
					}
