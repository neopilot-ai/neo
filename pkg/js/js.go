...
func Build(input EvalOptions) (esbuild.BuildResult, error) {
	...
	if outfile == "" {
		outfile = filepath.Join(input.Dir, ".neo", "platform", fmt.Sprintf("neo.config.%v.mjs", time.Now().UnixMilli()))
	}
...
` + input.Banner,
		...
		NodePaths: []string{
			filepath.Join(input.Dir, ".neo", "platform", "node_modules"),
		},
		Plugins: []esbuild.Plugin{
			{
				...
				Setup: func(build esbuild.PluginBuild) {
					build.OnResolve(esbuild.OnResolveOptions{Filter: ".*"}, func(args esbuild.OnResolveArgs) (esbuild.OnResolveResult, error) {
						if input.Globals == "" && filepath.Base(args.Importer) == "neo.config.ts" && args.Kind == esbuild.ResolveJSImportStatement {
				...
				Setup: func(build esbuild.PluginBuild) {
					build.OnLoad(esbuild.OnLoadOptions{Filter: `\.(js|ts|jsx|tsx)$`},
						func(args esbuild.OnLoadArgs) (esbuild.OnLoadResult, error) {
							if filepath.HasPrefix(args.Path, filepath.Join(input.Dir, ".neo")) {
							...
							if !strings.Contains(args.Path, ".neo") {
	...
	os.WriteFile(filepath.Join(input.Dir, ".neo", "esbuild.json"), []byte(analysis), 0644)
