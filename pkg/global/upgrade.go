...
func Upgrade(existingVersion string, nextVersion string) (string, error) {
	...
	if nextVersion == "" {
		resp, err := http.Get("https://api.github.com/repos/neopilot-ai/neo/releases/latest")
		if err != nil {
			return "", err
		}
	...
	url := "https://github.com/neopilot-ai/neo/releases/download/" + nextVersion + "/neo-" + filename
	...
	sstBinPath := filepath.Join(homeDir, ".neo", "bin")
	...
	if err := os.Chmod(filepath.Join(sstBinPath, "neo"), 0755); err != nil {
		return "", err
	}
...
func UpgradeNode(existingVersion string, nextVersion string) (map[string]string, error) {
	...
	// Ensure we only replace an existing "neo" entry in dependencies/devDependencies of package.json
	re := regexp.MustCompile(`(("dependencies"|"devDependencies")\s*:\s*{[^}]*"neo"\s*:\s*)"[^"]*"`)
	for _, file := range files {
		if strings.HasPrefix(file, ".neo") {
			continue
		}
