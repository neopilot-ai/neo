...
func Generate(root string, links common.Links) error {
	...
	rootEnv := filepath.Join(root, "neo-env.d.ts")
	for _, packageJson := range packageJsons {
		...
		envPath := filepath.Join(filepath.Dir(packageJson), "neo-env.d.ts")
		...
		if data.Dependencies["@cloudflare/workers-types"] != "" || data.DevDependencies["@cloudflare/workers-types"] != "" {
			...
			envFile.WriteString("import \"neo\"\n")
			envFile.WriteString("declare module \"neo\" {\n")
		...
		if rootEnv == envPath {
			envFile.Write([]byte("declare module \"neo\" {\n"))
