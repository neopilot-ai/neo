...
func CmdInit(cli *cli.Cli) error {
	if _, err := os.Stat("neo.config.ts"); err == nil {
		color.New(color.FgRed, color.Bold).Print("×")
		color.New(color.FgWhite, color.Bold).Println("  NEO project already exists")
		return nil
	}
	...
	case slices.ContainsFunc(hints, func(s string) bool { return strings.HasPrefix(s, "next.config") }):
		...
		fmt.Println("   - create an neo.config.ts")
		fmt.Println("   - modify the tsconfig.json")
		fmt.Println("   - add neo to package.json")
		template = "nextjs"
		break
