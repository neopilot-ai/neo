...
func New(ctx context.Context, cancel context.CancelFunc, root *Command, version string) (*Cli, error) {
	...
	flag.CommandLine.Init("neo", flag.ContinueOnError)
			...
			if stage == "" {
				...
				if stage == "" {
					if !term.IsTerminal(int(os.Stdout.Fd())) {
						return "", util.NewReadableError(nil, "No stage specified. Pass it in with --stage or set the NEO_STAGE environment variable. If this is a personal stage it can be set in `.neo/stage`.")
					}
					err := huh.NewForm(
						huh.NewGroup(
