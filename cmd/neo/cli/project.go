...
var logFile = (func() *os.File {
	tmpPath := flag.NEO_LOG
	if tmpPath == "" {
		tmpPath = filepath.Join(os.TempDir(), fmt.Sprintf("neo-%s.log", time.Now().Format("2006-01-02-15-04-05")))
	}
...
func (c *Cli) Discover() (string, error) {
	...
	if err != nil {
		return "", util.NewReadableError(err, "Could not find neo.config.ts")
	}
	return match, nil
}
...
func (c *Cli) configureLog() {
	...
	if c.Bool("print-logs") || flag.NEO_PRINT_LOGS {
