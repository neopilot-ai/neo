...
var CmdSecretList = &cli.Command{
	...
	Description: cli.Description{
		...
		Long: strings.Join([]string{
			...
			"neo secret list --stage production",
			"```",
			"",
			"List only the fallback secrets.",
			"",
			"```bash frame=\"none\" frame=\"none\"",
			"neo secret list --fallback",
			"```",
		}, "\n"),
	},
	Examples: []cli.Example{
		{
			Content: "neo secret list --stage production",
			Description: cli.Description{
...
var CmdSecretLoad = &cli.Command{
	...
	Description: cli.Description{
		...
		Long: strings.Join([]string{
			...
			"",
			"```bash frame=\"none\"",
			"neo secret load --stage production ./prod.env",
			"```",
			"",
			"Set these secrets as _fallback_ values.",
			"",
			"```bash frame=\"none\" frame=\"none\"",
			"neo secret load ./secrets.env --fallback",
			"```",
			"",
			...
			"",
			"```bash frame=\"none\"",
			"neo secret list > ./secrets.env",
			"neo secret load --stage production ./secrets.env",
			"```",
			"",
	...
	Examples: []cli.Example{
		{
			Content: "neo secret load ./secrets.env",
			Description: cli.Description{
				Short: "Loads all secrets from the file",
			},
		},
		{
			Content: "neo secret load ./prod.env --stage production",
			Description: cli.Description{
	...
	Run: func(c *cli.Cli) error {
		...
		ui.Success("Run \"neo deploy\" to update.")
		return nil
	},
}
...
var CmdSecretSet = &cli.Command{
	...
	Description: cli.Description{
		...
		Long: strings.Join([]string{
			...
			"",
			":::tip",
			"If you are not running `neo dev`, you'll need to `neo deploy` to apply the secret.",
			":::",
			"",
			"For example, set the `neo.Secret` called `StripeSecret` to `123456789`.",
			"",
			"```bash frame=\"none\"",
			"neo secret set StripeSecret dev_123456789",
			"```",
			"",
			"Optionally, set the secret in a specific stage.",
			"",
			"```bash frame=\"none\"",
			"neo secret set StripeSecret prod_123456789 --stage production",
			"```",
			"",
			"You can also set a _fallback_ value for a secret with `--fallback`.",
			"",
			"```bash frame=\"none\"",
			"neo secret set StripeSecret dev_123456789 --fallback",
			"```",
			"",
	...
	Examples: []cli.Example{
		{
			Content: "neo secret set StripeSecret 123456789",
			Description: cli.Description{
				Short: "Set the StripeSecret to 123456789",
			},
		},
		{
			Content: "neo secret set StripeSecret < tmp.txt",
			Description: cli.Description{
				Short: "Set the StripeSecret to contents of tmp.txt",
			},
		},
		{
			Content: "neo secret set StripeSecret productionsecret --stage production",
			Description: cli.Description{
	...
	Run: func(c *cli.Cli) error {
		...
		url, _ := server.Discover(p.PathConfig(), p.App().Stage)
		suffix := " Run \"neo deploy\" to update."
		if url != "" {
			suffix = ""
			dev.Deploy(c.Context, url)
		}
...
var CmdSecretRemove = &cli.Command{
	...
	Description: cli.Description{
		...
		Long: strings.Join([]string{
			"Remove a secret.",
			"",
			"For example, remove the `neo.Secret` called `StripeSecret`.",
			"",
			"```bash frame=\"none\" frame=\"none\"",
			"neo secret remove StripeSecret",
			"```",
			"",
			"Optionally, remove a secret in a specific stage.",
			"",
			"```bash frame=\"none\" frame=\"none\"",
			"neo secret remove StripeSecret --stage production",
			"```",
			"",
			"Remove the fallback value of the secret.",
			"",
			"```bash frame=\"none\" frame=\"none\"",
			"neo secret remove StripeSecret --fallback",
			"```",
		}, "\n"),
	},
	Args: []cli.Argument{
	...
	Examples: []cli.Example{
		{
			Content: "neo secret remove StripeSecret",
			Description: cli.Description{
				Short: "Remove the StripeSecret",
			},
		},
		{
			Content: "neo secret remove StripeSecret --stage production",
			Description: cli.Description{
