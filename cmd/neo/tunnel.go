...
var CmdTunnel = &cli.Command{
	...
	Description: cli.Description{
		...
		Long: strings.Join([]string{
			...
			":::",
			"",
			"If you are running `neo dev`, this tunnel will be started automatically under the",
			"_Tunnel_ tab in the sidebar.",
			"",
			":::tip",
			"This is automatically started when you run `neo dev`.",
			":::",
			"",
			"You can start this manually if you want to connect to a different stage.",
			"",
			"```bash frame=\"none\"",
			"neo tunnel --stage production",
			"```",
			"",
			"This needs a network interface on your local machine. You can create this",
			"with the `neo tunnel install` command.",
		}, "\n"),
	},
	Run: func(c *cli.Cli) error {
		if tunnel.NeedsInstall() {
			return util.NewReadableError(nil, "The neo tunnel needs to be installed or upgraded. Run `sudo neo tunnel install`")
		}
		...
		tunnelCmd.Env = append(
			os.Environ(),
			"NEO_SKIP_LOCAL=true",
			"NEO_SKIP_DEPENDENCY_CHECK=true",
			"SSH_PRIVATE_KEY="+tun.PrivateKey,
			"NEO_LOG="+strings.ReplaceAll(os.Getenv("NEO_LOG"), ".log", "_sudo.log"),
		)
		...
		if strings.Contains(string(output), "password is required") {
			return util.NewReadableError(nil, "Make sure you have installed the tunnel with `sudo neo tunnel install`")
		}
		return nil
	},
	Children: []*cli.Command{
		{
			...
			Description: cli.Description{
				...
				Long: strings.Join([]string{
					"Install the tunnel.",
					"",
					"To be able to create a tunnel, NEO needs to create a network interface on your local",
