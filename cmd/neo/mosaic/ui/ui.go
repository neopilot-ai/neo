...
func (u *UI) Event(unknown interface{}) {
	...
	case *aws.TaskMissingCommandEvent:
		u.printEvent(u.getColor(""), TEXT_DANGER_BOLD.Render(fmt.Sprintf("%-11s", "Missing")), fmt.Sprintf("Dev command not configured for the \"%s\" task. Set `dev.command` to configure how the task works in `neo dev`.", evt.Name))
	...
	case *project.ConcurrentUpdateEvent:
		u.reset()
		u.printEvent(TEXT_DANGER, "Locked", "A concurrent update was detected on the app. Run `neo unlock` to remove the lock and try again.")
	...
	case *project.CompleteEvent:
		...
		if len(evt.Errors) > 0 {
			...
			for _, status := range evt.Errors {
				...
				if ok {
					isNEOComponent := strings.Contains(status.URN, "::neo")
					if isNEOComponent {
...
func (u *UI) functionName(functionID string) string {
	...
	for _, resource := range u.complete.Resources {
		if resource.Type == "neo:aws:Function" && resource.URN.Name() == functionID {
			return resource.Outputs["_metadata"].(map[string]interface{})["handler"].(string)
		}
		if resource.Type == "neo:cloudflare:Worker" && resource.URN.Name() == functionID {
			return resource.Outputs["_metadata"].(map[string]interface{})["handler"].(string)
		}
	}
	return functionID
...
func (u *UI) header(version, app, stage string) {
	...
	u.println(
		TEXT_HIGHLIGHT_BOLD.Render("NEO "+version),
