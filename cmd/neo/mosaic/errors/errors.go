...
var transformers = []ErrorTransformer{
	...
	exact(project.ErrAppNameChanged, "The app name has changed.\n\nIf you want to rename the app, make sure to run `neo remove` to remove the old app first. Alternatively, remove the \".neo\" folder and try again.\n"),
	exact(project.ErrV2Config, "You are using neo v3 and this looks like an neo v2 config"),
	...
	exact(aws.ErrIoTDelay, "This aws account has not had iot initialized in it before which neo depends on. It may take a few minutes before it is ready."),
	...
	exact(server.ErrServerNotFound, "Could not find an `neo dev` session to connect to. Since you are running a command outside of the multiplexer be sure to start `neo dev` first."),
	exact(provider.ErrBucketMissing, "The state bucket is missing, it may have been accidentally deleted. Go to https://console.aws.amazon.com/systems-manager/parameters/%252Fsst%252Fbootstrap/description?tab=Table and check if the state bucket mentioned there exists. If it doesn't you can recreate it or delete the `/neo/bootstrap` key to force recreation."),
	exact(project.ErrProtectedStage, "Cannot remove protected stage. To remove a protected stage edit your neo.config.ts and remove the `protect` property."),
	exact(provider.ErrLockNotFound, "This app / stage is not locked"),
	exact(aws.ErrAppsyncNotReady, "NEO creates an appsync event api to power live lambda. After 10 seconds of waiting this cli could not connect to it."),
	exact(js.ErrTopLevelImport, "Your neo.config.ts has top level imports - this is not allowed. Move imports inside the function they are used and do a dynamic import: `const mod = await import(\"./mod\")`"),
	match(func(err *project.ErrBuildFailed) string {
		result := "Failed to build neo.config.ts"
	...
	match(func(err *project.ErrProviderVersionTooLow) string {
		return fmt.Sprintf("You specified version %s of the \"%s\" provider. NEO needs %s or higher.", err.Version, err.Name, err.Needed)
	}),
	match(func(err *project.ErrVersionMismatch) string {
		return fmt.Sprintf("You are using v%s which does not match v%s in your \"neo.config.ts\".", err.Needed, err.Received)
	}),
	func(err error) (bool, error) {
		...
		if strings.Contains(msg, "no EC2 IMDS role found") {
			return true, util.NewHintedError(err, "AWS credentials are not configured. Try configuring your profile in `~/.aws/config` and setting the `AWS_PROFILE` environment variable or specifying `providers.aws.profile` in your neo.config.ts")
		}
		return false, nil
	},
}
