...
import (
	...
	"github.com/neopilot-ai/neo/v3/cmd/neo/mosaic/monoplexer"
	"github.com/neopilot-ai/neo/v3/cmd/neo/mosaic/multiplexer"
	"github.com/neopilot-ai/neo/v3/cmd/neo/mosaic/socket"
...
func CmdMosaic(c *cli.Cli) error {
		...
		for {
			...
			case _, ok := <-evts:
				...
				if last == nil || diff(last.Env, nextEnv.Env) || last.Command != nextEnv.Command {
					...
					if child != "" && flag.NEO_LOG_CHILDREN {
	...
	os.Setenv("NEO_STAGE", p.App().Stage)
	...
	os.Setenv("NEO_SERVER", fmt.Sprintf("http://localhost:%v", server.Port))
	for name, a := range p.App().Providers {
		args := a
		switch name {
		case "aws":
			if flag.NEO_SKIP_APPSYNC {
				continue
			}
			wg.Go(func() error {
				defer c.Cancel()
	...
	if mode == "multi" {
		...
		multiEnv := append(
			c.Env(),
			fmt.Sprintf("NEO_SERVER=http://localhost:%v", server.Port),
			"NEO_STAGE="+p.App().Stage,
		)
		multi.AddProcess("deploy", []string{currentExecutable, "ui", "--filter=neo"}, "⑆", "NEO", "", false, true, append(multiEnv, "NEO_LOG="+p.PathLog("ui-deploy"))...)
		multi.AddProcess("function", []string{currentExecutable, "ui", "--filter=function"}, "λ", "Functions", "", false, true, append(multiEnv, "NEO_LOG="+p.PathLog("ui-function"))...)
		defer func() {
			multi.Exit()
		}()
		go func() {
					...
					case *project.CompleteEvent:
						for _, d := range evt.Devs {
							...
							multi.AddProcess(
								...
								true,
								d.Autostart,
								append([]string{"NEO_CHILD=" + d.Name}, multiEnv...)...,
							)
						}
						for name := range evt.Tunnels {
							multi.AddProcess("tunnel", []string{currentExecutable, "tunnel", "--stage", p.App().Stage}, "⇌", "Tunnel", "", true, true, append(
								multiEnv,
								"NEO_LOG="+p.PathLog("tunnel_"+name),
							)...)
						}
						if len(evt.Tasks) > 0 {
							multi.AddProcess("task", []string{currentExecutable, "ui", "--filter=task"}, "⧉", "Tasks", "", false, true, append(multiEnv, "NEO_LOG="+p.PathLog("ui-task"))...)
						}
						break
					}
				}
	...
	if mode == "mono" {
		mono := monoplexer.New()
		mono.AddProcess("deploy", []string{currentExecutable, "ui", "--filter=neo"}, "", "NEO")
