...
func CmdShell(c *cli.Cli) error {
	...
	cmd.Env = os.Environ()
	// Add NEO-specific environment variables
	...
	if target == "" {
		...
		if runtime.GOOS == "windows" && len(complete.Links) > 50 {
			...
			resourcesEnv := fmt.Sprintf("NEO_RESOURCES_JSON=%s", string(jsonData))
			cmd.Env = append(cmd.Env, resourcesEnv)
		} else {
			// Original approach: Add individual NEO resource environment variables
			for resource, value := range complete.Links {
				jsonValue, err := json.Marshal(value.Properties)
				if err != nil {
					return err
				}
				envVar := fmt.Sprintf("NEO_RESOURCE_%s=%s", resource, string(jsonValue))
				cmd.Env = append(cmd.Env, envVar)
			}
			appEnv := fmt.Sprintf("NEO_RESOURCE_App=%s", fmt.Sprintf(`{"name": "%s", "stage": "%s" }`, p.App().Name, p.App().Stage))
