...
var CmdDiff = &cli.Command{
	...
	Description: cli.Description{
		...
		Long: strings.Join([]string{
			...
			"",
			":::tip",
			"Run a `neo diff` to see what changes will be made when you deploy your app.",
			":::",
			"",
			...
			"",
			"Optionally, you can diff a specific component by passing in the name of the component from your `neo.config.ts`.",
			"",
			"```bash frame=\"none\"",
			"neo diff --target MyComponent",
			"```",
			"",
			"By default, this compares to the last deploy of the given stage as it would be",
			"deployed using `neo deploy`. But if you are working in dev mode using `neo dev`,",
	...
	Examples: []cli.Example{
		{
			Content: "neo diff --stage production",
			Description: cli.Description{
