...
impl Resource {
    pub fn init() -> Result<Self, ResourceError> {
        let key = BASE64_STANDARD.decode(env::var("NEO_KEY")?)?;
        let encrypted_data = std::fs::read(env::var("NEO_KEY_FILE")?)?;
        ...
        for (key, value) in env::vars() {
            if key.starts_with("NEO_RESOURCE_") {
                let result: Value = serde_json::from_str(&value)?;
                resources.insert(key.trim_start_matches("NEO_RESOURCE_").to_string(), result);
            }
        }
