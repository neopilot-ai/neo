...
/**
 * A list of OpenControl tools provided by NEO. Currently, it includes tools that
 * can:
 *
 * - Lists the resources in your NEO app.
 ...
 * import { tools } from "neo/opencontrol";
 *
 * const app = create({
 *   model: // ...
 *   tools: [...tools]
...
export const tools = [
  ...
  tool({
    name: "neo",
    description: "Get the resources in the current NEO app",
    async run() {
      const c = await client();
      const stateBucket = await getStateBucket();
      if (!stateBucket)
        throw new Error(
          "Failed to find the NEO state bucket in user's AWS account. Ask the user to make sure the AWS account has been bootstrapped with NEO."
        );

      const state = await getStateFile();
      if (!state)
        throw new Error(
          "Failed to find the NEO state file in user's AWS account."
        );
