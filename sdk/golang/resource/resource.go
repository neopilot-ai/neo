...
func init() {
	key, err := base64.StdEncoding.DecodeString(os.Getenv("NEO_KEY"))
	if err != nil {
		panic(err)
	}
	encryptedData, err := os.ReadFile(os.Getenv("NEO_KEY_FILE"))
	if err != nil {
...
func keys() {
	for _, item := range os.Environ() {
		...
		key := pair[0]
		value := pair[1]
		if strings.HasPrefix(key, "NEO_RESOURCE_") {
			var result map[string]interface{}
			err := json.Unmarshal([]byte(value), &result)
			if err != nil {
				panic(err)
			}
			resources[strings.TrimPrefix(key, "NEO_RESOURCE_")] = result
		}
	}
}
